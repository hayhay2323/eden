...
- `runtime/*` is the runtime task/control surface.
...
Use this for long-running runtime processes and operator-visible execution state.

Examples:

- `/api/runtime/tasks`
- `/api/runtime/tasks/:task_id`
- `/api/runtime/tasks/:task_id/status`

Rules:

- Runtime task payloads represent process/state lifecycle, not ontology objects.
- These routes are the preferred surface for monitoring HK/US runtimes, background work, and future workflow executors.
- Do not overload `agent/*` or `ontology/*` with process-heartbeat concerns.
...
- A dashboard or operator console:
  Prefer `ontology/*` for objects, `feed/*` for event rails, and `runtime/*` for process state.

- A detail page for a symbol/case/workflow:
  Start from the corresponding `ontology/*` object route, then follow `history_refs`.

- An operator workflow/review queue:
  Prefer the canonical queue surface `/api/ontology/:market/operator-work-items`, then follow the related object refs only when deeper drill-down is needed.
...
3. Start from `runtime/*` if the UI is process/task-centric.
4. Use `agent/*` only when matching an existing legacy consumer shape is required.
