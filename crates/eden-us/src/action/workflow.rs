...
impl UsActionWorkflow {
    ...
    pub fn from_setup(setup: &TacticalSetup, tick: u64, price: Option<Decimal>) -> Self {
        let symbol = match &setup.scope {
            eden::ontology::reasoning::ReasoningScope::Symbol(s) => s.clone(),
    ...
    pub fn confirm(&mut self) {
        ...
        self.notes
            .push("Workflow confirmed.".to_string());
    }
    ...
    pub fn execute(&mut self, price: Decimal) {
        ...
        self.notes
            .push(format!("Position executed at {price}."));
...
mod tests {
    ...
    use eden::ontology::reasoning::{DecisionLineage, ReasoningScope};
    ...
    fn stage_next_is_linear() {
        assert_eq!(UsActionStage::Suggested.next(), Some(UsActionStage::Confirmed));
        assert_eq!(UsActionStage::Confirmed.next(), Some(UsActionStage::Executed));
        assert_eq!(UsActionStage::Executed.next(), Some(UsActionStage::Monitoring));
        assert_eq!(UsActionStage::Monitoring.next(), Some(UsActionStage::Reviewed));
    ...
    fn review_advances_to_reviewed() {
        ...
        assert!(wf.notes.iter().any(|n| n.contains("closed with +10 profit")));
    }
