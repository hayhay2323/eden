...
impl UsPositionTracker {
    ...
    pub fn compute_degradation(
        ...
        let capital_flow_reversal =
            sign(current_dims.capital_flow_direction) != sign(fingerprint.entry_capital_flow)
        ...
        let momentum_decay =
            (fingerprint.entry_momentum.abs() - current_dims.price_momentum.abs()).max(Decimal::ZERO);
...
mod tests {
    ...
    fn no_degradation_when_unchanged() {
        ...
        let current = dims(dec!(0.5), dec!(0.5), dec!(0.5), Decimal::ZERO, Decimal::ZERO);
    ...
    fn exits_on_capital_flow_reversal_with_momentum_decay() {
        ...
        let current = dims(dec!(-0.3), dec!(0.5), dec!(0.5), Decimal::ZERO, Decimal::ZERO);
    ...
    fn exits_on_stale_ticks_held() {
        ...
        let current = dims(dec!(0.3), dec!(0.3), dec!(0.3), Decimal::ZERO, Decimal::ZERO);
    ...
    fn exits_on_volume_dry_up() {
        ...
        let current = dims(dec!(0.4), dec!(0.4), Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
        let deg = UsPositionTracker::compute_degradation(&fp, &current, 10);
    ...
    fn auto_exit_candidates_returns_only_failing_positions() {
        ...
        let snapshot = UsDimensionSnapshot {
            ...
            dimensions: {
                ...
                // AAPL: unchanged
                m.insert(sym("AAPL.US"), dims(dec!(0.4), dec!(0.4), dec!(0.4), Decimal::ZERO, Decimal::ZERO));
                // TSLA: composite collapses
                m.insert(sym("TSLA.US"), dims(dec!(-0.5), dec!(-0.5), dec!(-0.5), Decimal::ZERO, Decimal::ZERO));
                m
            },
        };
