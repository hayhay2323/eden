    ...
    for scope in &scopes {
        for template in TEMPLATES {
            ...
            let base_uncertainty =
                Decimal::ONE / (Decimal::ONE + evidence_count);
            let counter_confidence = counter_confidence.max(base_uncertainty);
            {
                hypotheses.push(Hypothesis {
                    ...
                    .with_trace_id(format!("hyp:{}:{}:counter", scope_id(scope), template.key)),
                    scope: scope.clone(),
                    statement: format!(
                    ...
                    invalidation_conditions: vec![InvalidationCondition {
                        description: format!("{} thesis holds — no reversal", template.family_label),
                        references: vec![],
                    }],
                    propagation_path_ids: vec![],
                    expected_observations: vec![
                        format!("{} signal should weaken", template.family_label),
                    ],
                });
            }
        }
...
fn has_signal_for_scope(
    ...
    signals.signals.iter().any(|s| {
        scope_matches(&convert_scope(&s.value.scope), scope)
            && signal_kind_in(&s.value.kind, kinds)
    })
}
...
mod tests {
    ...
    use crate::pipeline::signals::{
        UsDerivedSignalRecord, UsEventRecord,
    };
    ...
    fn pre_market_template_requires_dislocation_or_gap() {
        ...
        assert!(template_applicable(&TEMPLATES[0], &scope, &events, &signals));
    }
    ...
    fn cross_market_template_requires_divergence_or_propagation() {
        ...
        assert!(template_applicable(&TEMPLATES[1], &scope, &events, &signals));
    }
    ...
    fn momentum_template_requires_event_and_signal() {
        ...
        let empty_signals = UsDerivedSignalSnapshot { timestamp: ts(), signals: vec![] };
        assert!(!template_applicable(&TEMPLATES[2], &scope, &events, &empty_signals));

        // Event + signal → applicable
        let signals = UsDerivedSignalSnapshot {
            timestamp: ts(),
            signals: vec![make_signal("NVDA.US", UsDerivedSignalKind::StructuralComposite, dec!(0.5))],
        };
        assert!(template_applicable(&TEMPLATES[2], &scope, &events, &signals));
    }
    ...
    fn pre_market_supports_dislocation_contradicts_reversal() {
        assert_eq!(
            event_polarity(TEMPLATE_PRE_MARKET_POSITIONING, &UsEventKind::PreMarketDislocation),
            Some(EvidencePolarity::Supports)
        );
        assert_eq!(
            event_polarity(TEMPLATE_PRE_MARKET_POSITIONING, &UsEventKind::CapitalFlowReversal),
            Some(EvidencePolarity::Contradicts)
        );
    }
