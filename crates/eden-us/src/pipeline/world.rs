...
const MIN_COMPOSITE_FOR_BACKWARD: &str = "0.10";
...
pub fn derive_backward_snapshot(
    ...
    let min_composite: Decimal = MIN_COMPOSITE_FOR_BACKWARD
        .parse()
    ...
    let mut chains: Vec<UsBackwardChain> = decision
        .convergence_scores
        .iter()
        .filter(|(_, score)| score.composite.abs() >= min_composite)
        .filter_map(|(symbol, score)| {
            ...
            let neighbors = collect_aligned_neighbors(symbol, graph, score.composite > Decimal::ZERO);
            ...
            let evidence = build_evidence(symbol, &dims, score.sector_coherence, &neighbors, &sector_info, cm_signal.as_ref());
...
fn build_evidence(
    ...
    if !neighbors.is_empty() {
        ...
        let neighbor_weight = Decimal::from(neighbors.len() as i64)
            / Decimal::from(10);
        // Use composite direction inferred from the first dimension we have.
        let overall_dir = items
            .first()
            .map(|e| e.direction)
    ...
    if let (Some(sc), Some(info)) = (sector_coherence, sector_info) {
        if sc.abs() > Decimal::ZERO {
            ...
            let dir_str = if sc > Decimal::ZERO { "上行" } else { "下行" };
            items.push(UsBackwardEvidence {
...
/// Find graph neighbors moving in the same direction, filtered by similarity.
/// Only returns neighbors with above-median similarity to reduce noise.
/// Returns (symbols, mean_similarity) — mean_similarity is the weight for backward evidence.
...
mod tests {
    ...
    use eden::ontology::objects::SectorId;
    ...
    fn build_evidence_capital_flow_only() {
        let dims = make_dims(dec!(0.4), Decimal::ZERO, Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
    ...
    fn build_evidence_sorts_by_weight() {
        ...
        let dims = make_dims(dec!(0.3), dec!(0.8), Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
    ...
    fn build_evidence_cross_market_included() {
        let dims = make_dims(dec!(0.2), dec!(0.3), Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
    ...
    fn snapshot_derives_chain_for_strong_signal() {
        ...
        let decision = crate::graph::decision::UsDecisionSnapshot::compute(&graph, &[], 1);
    ...
    fn snapshot_includes_cross_market_in_evidence() {
        ...
        let decision = crate::graph::decision::UsDecisionSnapshot::compute(&graph, &cm_signals, 1);
        let snapshot = derive_backward_snapshot(&decision, &graph, &cm_signals, &HashMap::new());
    ...
    fn chains_sorted_by_confidence_descending() {
        let graph = make_graph(vec![
            (sym("AAPL.US"), make_dims(dec!(0.2), dec!(0.3), dec!(0.0), dec!(0.0), dec!(0.0))),
            (sym("NVDA.US"), make_dims(dec!(0.7), dec!(0.9), dec!(0.5), dec!(0.4), dec!(0.3))),
        ]);
        let decision = crate::graph::decision::UsDecisionSnapshot::compute(&graph, &[], 1);
        let snapshot = derive_backward_snapshot(&decision, &graph, &[], &HashMap::new());
    ...
    fn primary_driver_matches_highest_weight_evidence() {
        ...
        let chain = snapshot.chains.iter().find(|c| c.symbol == sym("TSLA.US")).unwrap();
