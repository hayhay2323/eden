...
impl UsDecisionSnapshot {
    pub fn compute(
        ...
        for symbol in graph.stock_nodes.keys() {
            if let Some(score) =
                UsConvergenceScore::compute(symbol, graph, cross_market_signals)
            {
...
mod tests {
    ...
    fn convergence_basic_positive() {
        let g = make_graph(vec![
            (sym("AAPL.US"), make_dims(dec!(0.3), dec!(0.5), dec!(0.2), dec!(0.1), dec!(0.4))),
        ]);
    ...
    fn convergence_with_cross_market() {
        let g = make_graph(vec![
            (sym("BABA.US"), make_dims(dec!(0.1), dec!(0.2), dec!(0.1), dec!(0.05), dec!(0))),
        ]);
    ...
    fn convergence_no_cross_market_for_non_dual() {
        let g = make_graph(vec![
            (sym("TSLA.US"), make_dims(dec!(0.5), dec!(0.8), dec!(0.3), dec!(0), dec!(0.1))),
        ]);
        let cm_signals = vec![CrossMarketSignal {
            hk_symbol: sym("9988.HK"),
            us_symbol: sym("BABA.US"),
    ...
    fn convergence_with_sector() {
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.3), dec!(0.5), dec!(0.2), dec!(0.1), dec!(0.4))),
            (sym("MSFT.US"), make_dims(dec!(0.2), dec!(0.4), dec!(0.1), dec!(0.05), dec!(0.3))),
        ]);
    ...
    fn convergence_zero_dims() {
        let g = make_graph(vec![
            (sym("AAPL.US"), make_dims(dec!(0), dec!(0), dec!(0), dec!(0), dec!(0))),
        ]);
        let score = UsConvergenceScore::compute(&sym("AAPL.US"), &g, &[]).unwrap();
    ...
    fn regime_neutral_on_mixed_market() {
        let dims = HashMap::from([
            (sym("AAPL.US"), make_dims(dec!(0.1), dec!(0.2), dec!(0), dec!(0), dec!(0))),
            (sym("NVDA.US"), make_dims(dec!(-0.1), dec!(-0.2), dec!(0), dec!(0), dec!(0))),
        ]);
    ...
    fn sell_suggestion_for_negative_composite() {
        let g = make_graph(vec![
            (sym("NVDA.US"), make_dims(dec!(-0.4), dec!(-0.6), dec!(-0.3), dec!(-0.2), dec!(-0.1))),
        ]);
