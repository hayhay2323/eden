...
use eden::ontology::objects::{SectorId, Symbol};
use petgraph::visit::EdgeRef;
use petgraph::Direction as GraphDirection;
use rust_decimal::Decimal;

use crate::graph::graph::{UsEdgeKind, UsGraph, UsNodeKind};
use crate::graph::propagation::CrossMarketSignal;
use crate::pipeline::dimensions::UsDimensionSnapshot;
...
fn std_dev(values: &[Decimal]) -> Decimal {
    ...
    eden::math::decimal_sqrt(variance)
}
...
fn compute_pressures(
    ...
    results.sort_by(|a, b| b.capital_flow_pressure.abs().cmp(&a.capital_flow_pressure.abs()));
    results
}
    ...
    for (_, members) in &components {
        ...
        let (stability, matched_age) = if prev_clusters.is_empty() {
            ...
            for (idx, prev_set) in prev_clusters.iter().enumerate() {
                ...
                if union_size > 0 {
                    let j =
                        Decimal::from(intersection as i64) / Decimal::from(union_size as i64);
                    if j > best_jaccard {
                        best_jaccard = j;
...
fn compute_stress_index(
    ...
    let pressure_vals: Vec<Decimal> = pressures
        .iter()
        .map(|p| p.capital_flow_pressure)
        .collect();
    let pressure_dispersion = std_dev(&pressure_vals);

    // 2. Momentum consensus: fraction of stocks sharing the same momentum sign
    let momentum_vals: Vec<Decimal> = dims
        .dimensions
        .values()
        .map(|d| d.price_momentum)
        .collect();
...
fn compute_cross_market_anomalies(
    ...
    for signal in cross_market {
        ...
        let divergence =
            (signal.propagation_confidence - actual_momentum).abs();
...
mod tests {
    ...
    use super::*;
    use crate::graph::graph::UsGraph;
    use crate::pipeline::dimensions::{UsDimensionSnapshot, UsSymbolDimensions};
    use eden::ontology::objects::{SectorId, Symbol};
    ...
    fn pressure_values_match_dimensions() {
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.6), dec!(0.4), dec!(0.3), dec!(0), dec!(0))),
            (sym("MSFT.US"), make_dims(dec!(-0.3), dec!(-0.2), dec!(0.1), dec!(0), dec!(0))),
        ]);
    ...
    fn sector_rotation_only_above_median() {
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.8), dec!(0.7), dec!(0.5), dec!(0), dec!(0))),
            (sym("MSFT.US"), make_dims(dec!(0.7), dec!(0.6), dec!(0.4), dec!(0), dec!(0))),
            (sym("XOM.US"),  make_dims(dec!(-0.5), dec!(-0.4), dec!(0.1), dec!(0), dec!(0))),
            (sym("CVX.US"),  make_dims(dec!(-0.6), dec!(-0.3), dec!(0.2), dec!(0), dec!(0))),
        ]);
    ...
    fn sector_rotation_spread_correct() {
        ...
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.9), dec!(0.8), dec!(0.5), dec!(0), dec!(0))),
            (sym("XOM.US"),  make_dims(dec!(0.1), dec!(0.1), dec!(0.1), dec!(0), dec!(0))),
            (sym("GS.US"),   make_dims(dec!(-0.7), dec!(-0.6), dec!(0.2), dec!(0), dec!(0))),
        ]);
        let sector_map = HashMap::from([
            (sym("AAPL.US"), sector("tech")),
            (sym("XOM.US"),  sector("energy")),
    ...
    fn cluster_age_filter_requires_3_ticks() {
        ...
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.8), dec!(0.7), dec!(0.5), dec!(0.1), dec!(0.2))),
            (sym("MSFT.US"), make_dims(dec!(0.8), dec!(0.7), dec!(0.5), dec!(0.1), dec!(0.2))),
        ]);
        let graph = make_graph(&snap, &HashMap::new());
    ...
    fn stress_index_consensus_calculation() {
        ...
        let snap = make_snapshot(vec![
            (sym("AAPL.US"), make_dims(dec!(0.3), dec!(0.5), dec!(0.2), dec!(0), dec!(0))),
            (sym("MSFT.US"), make_dims(dec!(0.2), dec!(0.4), dec!(0.1), dec!(0), dec!(0))),
            (sym("NVDA.US"), make_dims(dec!(0.1), dec!(0.3), dec!(0.3), dec!(0), dec!(0))),
            (sym("TSLA.US"), make_dims(dec!(-0.4), dec!(-0.6), dec!(0.5), dec!(0), dec!(0))),
        ]);
        let graph = make_graph(&snap, &HashMap::new());
        let insights = UsGraphInsights::compute(&graph, &snap, &[], None, 1);
    ...
    fn stress_index_dispersion_zero_when_uniform() {
        ...
        let snap = make_snapshot(vec![
            (sym("A.US"), make_dims(dec!(0.5), dec!(0.4), dec!(0.3), dec!(0), dec!(0))),
            (sym("B.US"), make_dims(dec!(0.5), dec!(0.2), dec!(0.1), dec!(0), dec!(0))),
            (sym("C.US"), make_dims(dec!(0.5), dec!(0.3), dec!(0.2), dec!(0), dec!(0))),
        ]);
        let graph = make_graph(&snap, &HashMap::new());
        let insights = UsGraphInsights::compute(&graph, &snap, &[], None, 1);
