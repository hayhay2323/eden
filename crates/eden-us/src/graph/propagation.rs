...
use crate::watchlist::CROSS_MARKET_PAIRS;
...
fn time_decay(minutes_since_close: i64) -> Decimal {
    ...
    if minutes_since_close <= 0 {
        ...
        Decimal::ONE
            - Decimal::from(minutes_since_close) / Decimal::from(FULL_DECAY_MINUTES)
    }
}
...
mod tests {
    ...
    fn applies_time_decay_to_confidence() {
        let snap = make_snapshot(vec![
            make_hk_signal("9988.HK", dec!(0.6), dec!(0.8)),
        ]);
    ...
    fn ignores_zero_composite() {
        let snap = make_snapshot(vec![
            make_hk_signal("9988.HK", dec!(0), dec!(0.5)),
        ]);
    ...
    fn multiple_dual_listed_pairs() {
        ...
        let baba = signals.iter().find(|s| s.us_symbol == sym("BABA.US")).unwrap();
        assert_eq!(baba.propagation_confidence, dec!(0.5) * dec!(0.75));

        let jd = signals.iter().find(|s| s.us_symbol == sym("JD.US")).unwrap();
        assert_eq!(jd.propagation_confidence, dec!(-0.3) * dec!(0.75));

        let bidu = signals.iter().find(|s| s.us_symbol == sym("BIDU.US")).unwrap();
