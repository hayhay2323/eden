...
mod tests {
    ...
    use crate::temporal::record::UsTickRecord;
    ...
    fn make_tick(tick: u64, entries: Vec<(Symbol, UsSymbolSignals)>) -> UsTickRecord {
        ...
        UsTickRecord {
            ...
            timestamp: OffsetDateTime::UNIX_EPOCH
                + time::Duration::seconds(tick as i64),
            signals,
    ...
    fn stable_leader_no_flips() {
        let mut history = crate::temporal::buffer::UsTickHistory::new(10);
        // Three ticks where momentum is always the dominant dimension.
        history.push(make_tick(
            1,
            vec![(sym("NVDA.US"), make_signals(dec!(0.1), dec!(0.8), dec!(0.2), dec!(0.1), dec!(0.0)))],
        ));
        history.push(make_tick(
            2,
            vec![(sym("NVDA.US"), make_signals(dec!(0.1), dec!(0.7), dec!(0.3), dec!(0.1), dec!(0.0)))],
        ));
        history.push(make_tick(
            3,
            vec![(sym("NVDA.US"), make_signals(dec!(0.2), dec!(0.9), dec!(0.1), dec!(0.0), dec!(0.0)))],
        ));
    ...
    fn leadership_flip_recorded() {
        ...
        history.push(make_tick(
            1,
            vec![(sym("XPEV.US"), make_signals(dec!(0.1), dec!(0.8), dec!(0.0), dec!(0.0), dec!(0.0)))],
        ));
        // Tick 2: capital_flow takes over.
        history.push(make_tick(
            2,
            vec![(sym("XPEV.US"), make_signals(dec!(0.9), dec!(0.2), dec!(0.0), dec!(0.0), dec!(0.0)))],
        ));
        // Tick 3: capital_flow still leads.
        history.push(make_tick(
            3,
            vec![(sym("XPEV.US"), make_signals(dec!(0.7), dec!(0.3), dec!(0.0), dec!(0.0), dec!(0.0)))],
        ));
    ...
    fn multiple_flips_tracked() {
        let mut history = crate::temporal::buffer::UsTickHistory::new(10);
        // Tick 1: capital_flow
        history.push(make_tick(1, vec![(sym("BABA.US"), make_signals(dec!(0.8), dec!(0.1), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        // Tick 2: momentum
        history.push(make_tick(2, vec![(sym("BABA.US"), make_signals(dec!(0.1), dec!(0.9), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        // Tick 3: pre_post_market
        history.push(make_tick(3, vec![(sym("BABA.US"), make_signals(dec!(0.1), dec!(0.2), dec!(0.0), dec!(0.8), dec!(0.0)))]));
    ...
    fn recent_leaders_returns_distinct_in_recency_order() {
        let mut history = crate::temporal::buffer::UsTickHistory::new(10);
        history.push(make_tick(1, vec![(sym("AAPL.US"), make_signals(dec!(0.8), dec!(0.1), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        history.push(make_tick(2, vec![(sym("AAPL.US"), make_signals(dec!(0.8), dec!(0.1), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        history.push(make_tick(3, vec![(sym("AAPL.US"), make_signals(dec!(0.1), dec!(0.9), dec!(0.0), dec!(0.0), dec!(0.0)))]));
    ...
    fn empty_history_produces_no_timelines() {
        let history = crate::temporal::buffer::UsTickHistory::new(10);
        let timelines = compute_causal_timelines(&history);
    ...
    fn symbol_absent_from_some_ticks_is_handled() {
        let mut history = crate::temporal::buffer::UsTickHistory::new(10);
        // Tick 1: only AAPL
        history.push(make_tick(1, vec![(sym("AAPL.US"), make_signals(dec!(0.5), dec!(0.1), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        // Tick 2: both AAPL and NVDA
        history.push(make_tick(2, vec![
            (sym("AAPL.US"), make_signals(dec!(0.4), dec!(0.2), dec!(0.0), dec!(0.0), dec!(0.0))),
            (sym("NVDA.US"), make_signals(dec!(0.0), dec!(0.7), dec!(0.0), dec!(0.0), dec!(0.0))),
        ]));
    ...
    fn flip_event_carries_composite_at_flip() {
        let mut history = crate::temporal::buffer::UsTickHistory::new(10);
        history.push(make_tick(1, vec![(sym("JD.US"), make_signals(dec!(0.8), dec!(0.1), dec!(0.0), dec!(0.0), dec!(0.0)))]));
        // composite at tick 2 = (0.1 + 0.7 + 0 + 0 + 0) / 5 = 0.16
        history.push(make_tick(2, vec![(sym("JD.US"), make_signals(dec!(0.1), dec!(0.7), dec!(0.0), dec!(0.0), dec!(0.0)))]));
