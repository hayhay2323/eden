...
fn mean_reversion_fires_on_extreme_valuation_and_counter_momentum() {
    ...
    let profile = ScenarioBuilder::new("2331.HK")
        .confidence(dec!(0.50))
        .action("review")
    ...
    let mut kinds = profile
        .primary_mechanism
        .iter()
        .map(|item| item.kind)
        .collect::<Vec<_>>();
    kinds.extend(profile.competing_mechanisms.iter().map(|item| item.kind));

    assert!(
        kinds.contains(&MechanismCandidateKind::MeanReversionSnapback),
        "expected MeanReversionSnapback to remain in the mechanism set, got {:?}",
        kinds
    );
}
