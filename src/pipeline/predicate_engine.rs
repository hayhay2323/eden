...
use crate::ontology::{
    action_direction_from_title_prefix, action_direction_position_label, ActionDirection,
    ActionNode, AtomicPredicate, AtomicPredicateKind,
};
...
pub(super) fn case_direction(inputs: &PredicateInputs<'_>) -> ActionDirection {
    ...
    action_direction_from_title_prefix(&inputs.tactical_case.title)
        .unwrap_or(ActionDirection::Neutral)
}
...
pub(super) fn direction_label(direction: ActionDirection) -> &'static str {
    action_direction_position_label(direction)
}
