...
pub struct SymbolResidual {
    ...
    /// Fourth residual channel. HK uses institutional alignment here; US
    /// stores cross-market propagation residual in this legacy numeric slot
    /// and marks `dominant_dimension=CrossMarket` when it dominates.
...
fn detect_sector_clusters(residuals: &[SymbolResidual]) -> Vec<SectorResidualCluster> {
    ...
    for (sector, sector_residuals) in &by_sector {
        ...
        let mean_net: Decimal = sector_residuals
            .iter()
        ...
        for r in sector_residuals {
            match r.dominant_dimension {
                ...
                ResidualDimension::CrossMarket => dim_counts[4] += 1,
            }
        }
        let dominant = [
...
pub fn infer_hidden_forces(field: &ResidualField, observed_at: OffsetDateTime) -> Vec<Hypothesis> {
    ...
    for residual in field.residuals.iter().take(10) {
        ...
        let mut hypothesis = Hypothesis {
            ...
            kind: Some(crate::ontology::reasoning::HypothesisKind::HiddenForce),
            ...
            invalidation_conditions: vec![InvalidationCondition {
                description: format!("residual shrinks below 0.05 or reverses direction",),
                references: vec![],
            }],
        ...
        crate::ontology::reasoning::enrich_hypothesis_with_ontology_projection(&mut hypothesis);
    ...
    for pair in &field.divergent_pairs {
        let mut hypothesis = Hypothesis {
            // V3: hypothesis_id is for stable identity; kind is the
            // typed marker that downstream consumers should match on.
            hypothesis_id: format!(
                "hyp:hidden_connection:{}:{}",
                pair.symbol_a.0, pair.symbol_b.0,
            ),
            kind: Some(crate::ontology::reasoning::HypothesisKind::HiddenConnection),
...
fn residual_to_confidence(magnitude: Decimal) -> Decimal {
    ...
    let base = (magnitude * Decimal::from(2)).min(Decimal::new(75, 2));
...
pub struct HiddenForceTracker {
    ...
    pub symbol: Symbol,
    /// For "hidden_connection" trackers (pairs), the partner symbol.
    /// None for single-symbol "hidden_force" trackers. Populated at
    /// construction so consumers don't need to parse hypothesis_id
    /// strings to extract pair endpoints (eliminated rule-based
    /// `hypothesis_id.split(':')` parsing in crystallize).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub partner_symbol: Option<Symbol>,
...
impl HiddenForceVerificationState {
    ...
    pub fn tick(
        ...
        for hyp in current_hypotheses {
            // V3: typed marker enum. Replaces V2's
            // `hypothesis_id.contains("...")` string-pattern matching
            // with structural enum dispatch.
            use crate::ontology::reasoning::HypothesisKind;
            let is_force = matches!(hyp.kind, Some(HypothesisKind::HiddenForce));
            let is_connection = matches!(hyp.kind, Some(HypothesisKind::HiddenConnection));
            if !is_force && !is_connection {
                continue;
            }
            ...
            // Parse partner_symbol ONCE at construction for "hidden_connection"
            // hypotheses — replaces the rule-based hypothesis_id.split(':')
            // re-parsing previously done inside crystallize_confirmed_forces.
            let partner_symbol = if is_connection {
                // V2 Pass 2: hypothesis_id format for hidden-connection
                // hypotheses is "hyp:hidden_connection:{a}:{b}" — partner
                // symbol lives at parts[3].
                let parts: Vec<&str> = hyp.hypothesis_id.split(':').collect();
                if parts.len() >= 4 {
                    Some(Symbol(parts[3].to_string()))
                } else {
                    None
                }
            } else {
            ...
            self.trackers.push(HiddenForceTracker {
                ...
                symbol,
                partner_symbol,
                family_key: if is_connection {
                    "hidden_connection".to_string()
                } else {
                    "hidden_force".to_string()
                },
                initial_residual,
                initial_magnitude,
                initial_dimension,
        ...
        self.trackers
            .retain(|t| !t.verdict.is_terminal() || current_tick.saturating_sub(t.last_tick) < 10);
...
pub fn crystallize_confirmed_forces(state: &HiddenForceVerificationState) -> CrystallizationOutput {
    ...
    for tracker in state.confirmed_forces() {
        match tracker.family_key.as_str() {
            ...
            "hidden_connection" => {
                // Endpoints come from the typed `partner_symbol` field
                // populated at tracker construction. No more brittle
                // hypothesis_id string-parsing.
                let sym_a = tracker.symbol.clone();
                let sym_b = match &tracker.partner_symbol {
                    Some(s) => s.clone(),
                    None => continue,
                };
                {
                    ...
                    let confidence =
                        Decimal::from(tracker.persistence_streak.min(5)) * Decimal::new(15, 2); // 0.15 per tick, max 0.75
...
// ---------------------------------------------------------------------------
// Hub aggregation: structural roll-up of emergent edges
// ---------------------------------------------------------------------------
//
// Individual `EmergentGraphEdge`s describe a ↔ b pairwise anti/correlation.
// When one symbol participates in many such edges, it is behaving as a HUB —
// a node whose residuals are structurally divergent from a cluster of peers.
// `aggregate_hubs` rolls the edge list up by endpoint so the operator surface
// can see the *node*, not just the edges.

/// Rolled-up summary of one symbol's emergent-edge activity.
#[derive(Debug, Clone)]
pub struct HubSummary {
    pub symbol: Symbol,
    pub anticorr_degree: usize,
    pub corr_degree: usize,
    /// Top peer symbol names sorted by `strength * persistence_ticks` descending.
...
impl HubSummary {
    pub fn total_degree(&self) -> usize {
        self.anticorr_degree + self.corr_degree
    }
}

/// Group emergent edges by endpoint and return hubs with degree >= min_degree,
/// sorted by total degree (desc). An edge a↔b counts toward BOTH a and b.
/// `peers` inside each HubSummary is capped at top 5 by strength × persistence.
pub fn aggregate_hubs(edges: &[EmergentGraphEdge], min_degree: usize) -> Vec<HubSummary> {
    if edges.is_empty() {
        return Vec::new();
    }

    // For each symbol, collect the list of (peer_name, edge_type, strength,
    // persistence_ticks) entries contributed by edges it participates in.
    struct PeerEntry {
        peer: String,
        edge_type: EmergentEdgeType,
        strength: Decimal,
        persistence_ticks: u64,
    }
    let mut by_symbol: HashMap<Symbol, Vec<PeerEntry>> = HashMap::new();
    for edge in edges {
        by_symbol
            .entry(edge.symbol_a.clone())
            .or_default()
            .push(PeerEntry {
                peer: edge.symbol_b.0.clone(),
                edge_type: edge.edge_type,
                strength: edge.strength,
                persistence_ticks: edge.persistence_ticks,
            });
        by_symbol
            .entry(edge.symbol_b.clone())
            .or_default()
            .push(PeerEntry {
                peer: edge.symbol_a.0.clone(),
                edge_type: edge.edge_type,
                strength: edge.strength,
                persistence_ticks: edge.persistence_ticks,
            });
    ...
    let mut hubs: Vec<HubSummary> = by_symbol
        .into_iter()
        .filter(|(_, entries)| entries.len() >= min_degree)
        .map(|(symbol, mut entries)| {
            let total = entries.len();
            let anticorr_degree = entries
                .iter()
                .filter(|e| e.edge_type == EmergentEdgeType::HiddenAntiCorrelation)
                .count();
            let corr_degree = entries
                .iter()
                .filter(|e| e.edge_type == EmergentEdgeType::HiddenCorrelation)
                .count();
            let max_streak = entries
                .iter()
                .map(|e| e.persistence_ticks)
                .max()
                .unwrap_or(0);
            let strength_sum: Decimal = entries.iter().map(|e| e.strength).sum();
            let mean_strength = if total > 0 {
                (strength_sum / Decimal::from(total)).round_dp(4)
            } else {
                Decimal::ZERO
            };

            // Rank peers by strength × persistence_ticks; tiebreak by peer name asc.
            entries.sort_by(|a, b| {
                let score_a = a.strength * Decimal::from(a.persistence_ticks);
                let score_b = b.strength * Decimal::from(b.persistence_ticks);
                score_b.cmp(&score_a).then_with(|| a.peer.cmp(&b.peer))
            });
            let peers: Vec<String> = entries.into_iter().take(5).map(|e| e.peer).collect();
...
/// Format a HubSummary as a one-line `risk_notes` entry for attachment to a
/// TacticalSetup. Honest closure of the hub → setup loop: we attach the
/// observation, we don't impose a direction rule. hub is symmetric coupling
/// without lead-lag, so modulating confidence from it is guesswork. Instead
/// we tag setups that land on hub symbols so (a) operator sees it inline,
/// (b) case_realized_outcome records it, (c) outcome_history can later
/// learn empirical hit rate per hub shape.
pub fn hub_member_risk_note(hub: &HubSummary) -> String {
    format!(
        "hub_member: degree={} anticorr={} corr={} streak={} strength={:.2} peers={}",
        hub.total_degree(),
        hub.anticorr_degree,
        hub.corr_degree,
        hub.max_streak,
    ...
    for tracker in &state.trackers {
        ...
        let explanation = match verdict {
            OptionVerdict::Confirms => format!(
                ...
                signals
                    .iter()
                    .filter(|s| s.interpretation.contains("confirms"))
                    .count(),
            ),
            OptionVerdict::Contradicts => format!(
                ...
                signals
                    .iter()
                    .filter(|s| s.interpretation.contains("contradicts"))
                    .count(),
            ),
...
fn evaluate_tracker(tracker: &HiddenForceTracker) -> HiddenForceVerdict {
    ...
    if history.len() >= 2
        && initial_sign_positive != latest_sign_positive
        && latest.abs() > Decimal::new(3, 2)
    {
    ...
    let earlier_avg = if history.len() >= 5 {
        let first_portion: Decimal = history[..history.len() - 3].iter().take(3).sum();
...
mod tests {
    ...
    fn residual_dimension_label() {
        ...
        assert_eq!(ResidualDimension::CrossMarket.label(), "cross_market");
    }
    ...
    fn infer_isolated_hidden_force() {
        ...
        assert!(hypotheses[0].hypothesis_id.contains("hidden_force"));
    ...
    fn infer_connection_hidden_force() {
        ...
        let hypotheses = infer_hidden_forces(&field, OffsetDateTime::UNIX_EPOCH);
        assert_eq!(hypotheses.len(), 1);
        assert!(hypotheses[0].hypothesis_id.contains("hidden_connection"));
    ...
    fn make_hyp(id: &str, symbol: &str, family: &str) -> Hypothesis {
        // V3: family parameter selects the typed kind enum.
        let kind = match family {
            "hidden_force" => Some(crate::ontology::reasoning::HypothesisKind::HiddenForce),
            "hidden_connection" => {
                Some(crate::ontology::reasoning::HypothesisKind::HiddenConnection)
            }
            _ => None,
        };
        Hypothesis {
    ...
    fn verification_confirms_persistent_residual() {
        ...
        let hyp = make_hyp("hyp:hidden_force:test", "700.HK", "hidden_force");
    ...
    fn verification_invalidates_on_reversal() {
        let mut state = HiddenForceVerificationState::default();
        let hyp = make_hyp("hyp:hidden_force:rev", "700.HK", "hidden_force");
    ...
    fn verification_resolves_when_residual_vanishes() {
        let mut state = HiddenForceVerificationState::default();
        let hyp = make_hyp("hyp:hidden_force:vanish", "700.HK", "hidden_force");
    ...
    fn confidence_adjustments_boost_confirmed() {
        let mut state = HiddenForceVerificationState::default();
        let hyp = make_hyp("hyp:hidden_force:boost", "700.HK", "hidden_force");
    ...
    fn prunes_terminal_trackers() {
        let mut state = HiddenForceVerificationState::default();
        let hyp = make_hyp("hyp:hidden_force:prune", "700.HK", "hidden_force");
    ...
    fn make_confirmed_tracker(symbol: &str, positive: bool) -> HiddenForceTracker {
        HiddenForceTracker {
            ...
            symbol: sym(symbol),
            partner_symbol: None,
            family_key: "hidden_force".to_string(),
    ...
    fn option_confirms_positive_force_with_bullish_skew() {
        ...
        let options = vec![make_option_surface(
            "AAPL.US",
            dec!(25),
            dec!(22),
            10000,
    ...
    // --- aggregate_hubs tests ---

    fn anti_edge(a: &str, b: &str, strength: Decimal, streak: u64) -> EmergentGraphEdge {
        EmergentGraphEdge {
            symbol_a: sym(a),
            symbol_b: sym(b),
            edge_type: EmergentEdgeType::HiddenAntiCorrelation,
            strength,
            persistence_ticks: streak,
            evidence_summary: format!("test edge {}↔{}", a, b),
        }
    }

    #[test]
    fn aggregate_hubs_empty_input() {
        assert!(aggregate_hubs(&[], 3).is_empty());
    }

    #[test]
    fn aggregate_hubs_low_degree_filtered() {
        // Two disjoint edges → every endpoint has degree=1, none pass min_degree=3.
        let edges = vec![
            anti_edge("A", "B", dec!(0.5), 5),
            anti_edge("C", "D", dec!(0.6), 7),
        ];
        assert!(aggregate_hubs(&edges, 3).is_empty());
    }

    #[test]
    fn aggregate_hubs_single_hub_five_peers() {
        // 6068 connected to 5 distinct peers → degree=5, all HiddenAntiCorrelation.
        let edges = vec![
            anti_edge("6068.HK", "A.HK", dec!(0.75), 10),
            anti_edge("6068.HK", "B.HK", dec!(0.75), 20),
            anti_edge("6068.HK", "C.HK", dec!(0.60), 4),
            anti_edge("6068.HK", "D.HK", dec!(0.75), 36),
            anti_edge("6068.HK", "E.HK", dec!(0.75), 15),
        ];
        let hubs = aggregate_hubs(&edges, 3);
        // 6068 is the only hub; peers (A–E) each have degree=1 so filtered out.
        assert_eq!(hubs.len(), 1);
        let hub = &hubs[0];
        assert_eq!(hub.symbol.0, "6068.HK");
        assert_eq!(hub.anticorr_degree, 5);
        assert_eq!(hub.corr_degree, 0);
        assert_eq!(hub.max_streak, 36);
        // Top peer by strength*streak: D (0.75*36=27) > B (0.75*20=15) > E (0.75*15=11.25)
        // > A (0.75*10=7.5) > C (0.60*4=2.4)
        assert_eq!(hub.peers, vec!["D.HK", "B.HK", "E.HK", "A.HK", "C.HK"]);
    }

    #[test]
    fn aggregate_hubs_mutual_hub_bilateral_counting() {
        // H1 connected to 3 peers AND H2 connected to 3 peers, sharing edge H1↔H2.
        // H1: edges to H2, X, Y → degree=3.
        // H2: edges to H1, P, Q → degree=3.
        let edges = vec![
            anti_edge("H1", "H2", dec!(0.75), 20),
            anti_edge("H1", "X", dec!(0.70), 10),
            anti_edge("H1", "Y", dec!(0.70), 10),
            anti_edge("H2", "P", dec!(0.70), 10),
            anti_edge("H2", "Q", dec!(0.70), 10),
        ];
        let hubs = aggregate_hubs(&edges, 3);
        assert_eq!(hubs.len(), 2);
        let h1 = hubs.iter().find(|h| h.symbol.0 == "H1").expect("H1 hub");
        let h2 = hubs.iter().find(|h| h.symbol.0 == "H2").expect("H2 hub");
        assert_eq!(h1.anticorr_degree, 3);
        assert_eq!(h2.anticorr_degree, 3);
        // H2 must appear in H1's peer list and vice versa.
        assert!(h1.peers.contains(&"H2".to_string()));
        assert!(h2.peers.contains(&"H1".to_string()));
        // The H1↔H2 edge with streak=20 should rank these peers highest.
        assert_eq!(h1.peers[0], "H2");
        assert_eq!(h2.peers[0], "H1");
    }

    #[test]
    fn hub_member_risk_note_shape() {
        let edges = vec![
            anti_edge("AES.US", "BABA.US", dec!(0.75), 10),
            anti_edge("AES.US", "BIDU.US", dec!(0.70), 10),
            anti_edge("AES.US", "JD.US", dec!(0.65), 10),
        ];
        let hubs = aggregate_hubs(&edges, 3);
        assert_eq!(hubs.len(), 1);
        let note = hub_member_risk_note(&hubs[0]);
        assert!(
            note.starts_with("hub_member:"),
            "unexpected prefix: {}",
            note
        );
        assert!(note.contains("degree=3"), "missing degree: {}", note);
        assert!(note.contains("anticorr=3"), "missing anticorr: {}", note);
        assert!(note.contains("streak=10"), "missing streak: {}", note);
        assert!(note.contains("BABA.US"), "missing peer: {}", note);
    }
}
