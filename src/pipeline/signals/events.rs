    ...
    pub fn detect(
        ...
        for event in &mut events {
            ...
            if !attr.is_empty() {
                ...
                event.provenance.inputs.retain(|i| !i.starts_with("attr:"));
                event
                    .provenance
                    .inputs
                    .extend(attr.iter().map(|s| s.to_string()));
            }
        }
    ...
    for event in &mut snapshot.events {
        ...
        if has_local_scope {
            event
                .provenance
                .inputs
                .retain(|i| !i.starts_with("attr:scope=") && !i.starts_with("attr:driver="));
            event.provenance.inputs.extend(
                super::types::attribution_inputs_for_kind(&MarketEventKind::SharedHolderAnomaly)
                    .iter()
                    .map(|s| s.to_string()),
            );
        }
