...
pub struct PressureVortex {
    ...
    /// If this vortex was detected from an edge violation (a strong learned edge
    /// that should have propagated but didn't), this contains the source symbol.
    /// None = traditional time-scale tension vortex.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub edge_violation_source: Option<Symbol>,
}
...
impl PressureField {
    ...
    pub fn tick(
        ...
        let mut vortices = detect_vortices(tick_layer, hour_layer);

        // 8. Detect edge violation vortices: strong learned edges that should
        //    propagate but didn't. A weight=1.4 edge where source has tension
        //    but target doesn't = the graph's expectation was violated.
        let edge_violations = detect_edge_violations(minute_layer, brain, edge_ledger);
        vortices.extend(edge_violations);
        vortices.sort_by(|a, b| b.tension.cmp(&a.tension));
        vortices.truncate(15);
    ...
    pub fn tick_us(
        ...
        let hour_layer = self.layers.get(&TimeScale::Hour).unwrap();
        let mut vortices = detect_vortices(tick_layer, hour_layer);
        let edge_violations = detect_edge_violations_us(minute_layer, graph, edge_ledger);
        vortices.extend(edge_violations);
        vortices.sort_by(|a, b| b.tension.cmp(&a.tension));
        vortices.truncate(15);
        self.vortices = vortices;
    ...
    fn learn_from_hour_deltas(
        ...
        for (symbol, edge_attributions) in &propagated_pressures.edge_attributions {
            ...
            let total_magnitude: Decimal =
                edge_attributions.iter().map(|edge| edge.magnitude).sum();
...
fn compute_local_pressure(dimensions: &HashMap<Symbol, SymbolDimensions>) -> ScaledPressure {
...
fn propagate_pressure(
    ...
    for (symbol, &node_idx) in &brain.stock_nodes {
        ...
        for edge in brain
            .graph
            .edges_directed(node_idx, GraphDirection::Incoming)
        {
            match edge.weight() {
        ...
        if !propagated_channels.is_empty() {
            result
                .propagated
                .insert(symbol.clone(), propagated_channels);
        }
...
fn compute_anomalies(
    ...
    for (symbol, node) in &current.pressures {
        ...
        let base_convergence = baseline_node
            .map(|n| n.convergence)
...
fn detect_vortices(
    ...
    for (symbol, hour_node) in &hour_layer.pressures {
        ...
        let tension = (temporal_div.abs() + hour_conflict + max_channel_tension).round_dp(4);
    ...
    // Cross-sectional centering: rebase hour_direction against the
    // tick's vortex-universe median so direction reflects relative
    // strength, not absolute composite sign. See helper doc above.
    recenter_vortex_directions_by_median(&mut vortices);
...
// ── Step 6b: Detect edge violation vortices ──
//
// A learned edge with weight > 1.2 means "these two symbols historically move
// together." If source has material tension but target doesn't, the graph's
// expectation is violated. This is more informative than time-scale disagreement
// because it's based on LEARNED history, not fixed rules.

fn detect_edge_violations(
    minute_layer: &ScaledPressure,
    brain: &BrainGraph,
    edge_ledger: &EdgeLearningLedger,
) -> Vec<PressureVortex> {
    use petgraph::visit::EdgeRef;

    let tension_threshold = Decimal::new(5, 2); // source must have >= 0.05 composite
    let silence_threshold = Decimal::new(1, 2); // target must have < 0.01 composite
    let edge_strength_threshold = Decimal::new(12, 1); // learned weight must be > 1.2

    let mut violations = Vec::new();

    for (symbol, &node_idx) in &brain.stock_nodes {
        let source_pressure = minute_layer
            .pressures
            .get(symbol)
            .map(|n| n.composite.abs())
            .unwrap_or(Decimal::ZERO);

        if source_pressure < tension_threshold {
            continue;
        }

        // Check outgoing stock-to-stock edges
        for edge in brain
            .graph
            .edges_directed(node_idx, petgraph::Direction::Outgoing)
        {
            if let crate::graph::graph::EdgeKind::StockToStock(_) = edge.weight() {
                let target_idx = edge.target();
                let target_symbol = match &brain.graph[target_idx] {
                    crate::graph::graph::NodeKind::Stock(s) => &s.symbol,
                    _ => continue,
                };

                let edge_key = EdgeKey::StockToStock {
                    a: if symbol.0 < target_symbol.0 {
                        symbol.clone()
                    } else {
                        target_symbol.clone()
                    },
                    b: if symbol.0 < target_symbol.0 {
                        target_symbol.clone()
                    } else {
                        symbol.clone()
                    },
                ...
                let multiplier = edge_ledger.weight_multiplier(&edge_key);
                if multiplier < edge_strength_threshold {
                    continue; // only care about strong learned edges
                }

                let target_pressure = minute_layer
                    .pressures
                    .get(target_symbol)
                    .map(|n| n.composite.abs())
                    .unwrap_or(Decimal::ZERO);

                if target_pressure < silence_threshold {
                    // Strong edge, source has tension, target is silent = VIOLATION
                    let source_node = minute_layer.pressures.get(symbol).unwrap();
                    violations.push(PressureVortex {
                        symbol: target_symbol.clone(),
                        tension: (source_pressure * multiplier).round_dp(4),
                        cross_channel_conflict: Decimal::ZERO,
                        temporal_divergence: Decimal::ZERO,
                        hour_direction: source_node.composite.round_dp(4),
                        tick_direction: Decimal::ZERO,
                        tense_channels: vec![],
                        tense_channel_count: 0,
                        edge_violation_source: Some(symbol.clone()),
                    });
                }
            }
        }
    ...
    violations.sort_by(|a, b| b.tension.cmp(&a.tension));
    violations.truncate(5); // max 5 edge violation vortices per tick
    violations
}

fn detect_edge_violations_us(
    minute_layer: &ScaledPressure,
    graph: &UsGraph,
    edge_ledger: &EdgeLearningLedger,
) -> Vec<PressureVortex> {
    ...
    let tension_threshold = Decimal::new(5, 2);
    let silence_threshold = Decimal::new(1, 2);
    let edge_strength_threshold = Decimal::new(12, 1);
    ...
    for (symbol, &node_idx) in &graph.stock_nodes {
        ...
        for edge in graph
            ...
            .edges_directed(node_idx, petgraph::Direction::Outgoing)
        {
            if let crate::us::graph::graph::UsEdgeKind::StockToStock(_) = edge.weight() {
                let target_idx = edge.target();
                let target_symbol = match &graph.graph[target_idx] {
                    crate::us::graph::graph::UsNodeKind::Stock(s) => &s.symbol,
                    _ => continue,
                };
...
/// Re-center each vortex's `hour_direction` around the cross-sectional
/// median across the current tick's vortex universe. Without this, in
/// a broadly-red session every symbol's own composite is negative and
/// every vortex emits Short — there is no Long signal even when some
/// symbols are clearly outperforming the market. After centering, the
/// sign of `hour_direction` reflects relative strength against the
/// market median: positive = relatively strong (Long candidate),
/// negative = relatively weak (Short candidate). Captures intra-sector
/// dispersion / rebound leaders, which the per-symbol absolute
/// composite cannot.
fn recenter_vortex_directions_by_median(vortices: &mut [PressureVortex]) {
    if vortices.len() < 2 {
        return;
    }
    let mut sorted: Vec<Decimal> = vortices.iter().map(|v| v.hour_direction).collect();
    sorted.sort();
    let median = sorted[sorted.len() / 2];
    for v in vortices.iter_mut() {
        v.hour_direction = (v.hour_direction - median).round_dp(4);
    }
}
...
fn compute_us_local_pressure(dimensions: &HashMap<Symbol, UsSymbolDimensions>) -> ScaledPressure {
    let mut pressures = HashMap::new();
            ...
            .edges_directed(node_idx, GraphDirection::Incoming)
        {
            match edge.weight() {
                ...
                UsEdgeKind::CrossMarket(e) => {
                    ...
                    let edge_key = EdgeKey::StockToStock { a, b };
                    let weight = e.propagation_strength
                        * e.confidence
                        * edge_ledger.weight_multiplier(&edge_key);
                    if weight > Decimal::ZERO {
                        ...
                        let edge_channel_acc = edge_acc.entry(edge_key).or_default();
                        *edge_channel_acc
                            .entry(PressureChannel::CapitalFlow)
                            .or_default() += weighted;
                    }
                }
            }
...
mod tests {
    ...
    fn vortex_with_hour_dir(symbol: &str, hour_dir: Decimal) -> PressureVortex {
        PressureVortex {
            symbol: Symbol(symbol.into()),
            tension: dec!(0.5),
            cross_channel_conflict: dec!(0),
            temporal_divergence: dec!(0),
            hour_direction: hour_dir,
            tick_direction: dec!(0),
            tense_channels: vec![PressureChannel::CapitalFlow],
            tense_channel_count: 1,
            edge_violation_source: None,
        }
    }

    #[test]
    fn recenter_vortex_directions_centers_on_median_so_red_session_emits_long_too() {
        // Regression: previously vortex.hour_direction was the symbol's
        // own hour_composite sign. In a broadly red session every symbol's
        // composite is negative, so every vortex emitted Short. Cross-
        // sectional centering makes Long fire for symbols above the
        // market median (relatively strong) even when median itself is
        // negative — captures intra-sector dispersion / rebound leaders.
        let mut vortices = vec![
            vortex_with_hour_dir("STRONG.US", dec!(-0.05)), // above median
            vortex_with_hour_dir("MID.US", dec!(-0.30)),    // median
            vortex_with_hour_dir("WEAK1.US", dec!(-0.40)),
            vortex_with_hour_dir("WEAK2.US", dec!(-0.55)),
        ];
        recenter_vortex_directions_by_median(&mut vortices);
        // Sorted: -0.55, -0.40, -0.30, -0.05. Median index 2 = -0.30.
        assert_eq!(vortices[0].hour_direction, dec!(0.25)); // STRONG.US
    ...
    #[test]
    fn recenter_vortex_directions_skips_when_too_few_for_median() {
        let mut empty: Vec<PressureVortex> = Vec::new();
        recenter_vortex_directions_by_median(&mut empty);
        assert!(empty.is_empty());
        let mut single = vec![vortex_with_hour_dir("X.US", dec!(0.5))];
        recenter_vortex_directions_by_median(&mut single);
        assert_eq!(single[0].hour_direction, dec!(0.5)); // unchanged
    }
    ...
    fn local_pressure_from_dimensions() {
        ...
        assert_eq!(
            node.channels[&PressureChannel::CapitalFlow].local,
            dec!(0.3)
        );
        assert_eq!(
            node.channels[&PressureChannel::Institutional].local,
            dec!(0.7)
        );
