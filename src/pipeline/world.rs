...
impl WorldSnapshots {
    pub fn derive(
        ...
        let world_state =
            derive_world_state(events, derived_signals, insights, decision, reasoning);
...
fn derive_world_state(
    ...
    let mut sectors: HashMap<crate::ontology::SectorId, Vec<&crate::ontology::Hypothesis>> =
        HashMap::new();
    let market_regime = if insights.stress.composite_stress >= Decimal::new(45, 2) {
        "stress-dominant".to_string()
    } else if !reasoning.case_clusters.is_empty() {
        "narrative-fragmenting".to_string()
    } else {
        "locally-driven".to_string()
    };
