    ...
    pub fn compute(
        ...
        if portfolio.position_count >= self.max_positions {
            return SizeResult {
                ...
                reject_reason: Some(format!("已達最大持倉數 {}", self.max_positions)),
            };
        }
        ...
        let shares_raw = shares_by_risk.min(shares_by_notional).min(shares_by_sector);
        if shares_raw <= Decimal::ZERO {
            return SizeResult {
                quantity: 0,
                notional: Decimal::ZERO,
                equity_pct: Decimal::ZERO,
                risk_amount: Decimal::ZERO,
                reject_reason: Some("計算後數量非正值".into()),
            };
        }

        // Round down to lot size
        let Some(shares_u64) = shares_raw.floor().to_u64() else {
            return SizeResult {
                quantity: 0,
                notional: Decimal::ZERO,
                equity_pct: Decimal::ZERO,
                risk_amount: Decimal::ZERO,
                reject_reason: Some("計算後數量超出可交易範圍".into()),
            };
        };
        let lots = shares_u64 / lot_size;
...
mod tests {
    ...
    #[test]
    fn negative_shares_do_not_wrap_to_large_u64() {
        let sizer = PositionSizer::default();
        let mut p = portfolio(dec!(100000), 0);
        p.sector_exposure.insert("tech".into(), dec!(40000));
