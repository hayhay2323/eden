...
use crate::live_snapshot::spawn_write_json_snapshot_if_latest;
...
pub fn persist_projection_tick(envelope: ProjectionPersistenceEnvelope<'_>) {
    ...
    let market_slug = match envelope.market {
        CaseMarket::Hk => "hk",
        CaseMarket::Us => "us",
    };
    match build_operational_snapshot(
        ...
        Ok(snapshot) => spawn_write_json_snapshot_if_latest(
            format!("agent:{market_slug}"),
