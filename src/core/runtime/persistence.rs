...
#[cfg(feature = "persistence")]
fn latest_store_operation_versions() -> &'static Mutex<HashMap<String, i128>> {
    static VERSIONS: OnceLock<Mutex<HashMap<String, i128>>> = OnceLock::new();
...
#[cfg(feature = "persistence")]
pub async fn schedule_latest_store_operation<F, Fut, E>(
    store: &Option<EdenStore>,
    persistence_limit: &Arc<Semaphore>,
    config: &RuntimeInfraConfig,
    label: &'static str,
    ...
    E: Display + Send + 'static,
{
    ...
    {
        let mut versions = latest_store_operation_versions()
            .lock()
            .expect("latest store operation lock poisoned");
        match versions.get(&freshness_key).copied() {
            Some(latest) if latest > version => return,
            _ => {
                versions.insert(freshness_key.clone(), version);
            }
        }
    }
    let Ok(permit) = persistence_limit.clone().acquire_owned().await else {
    ...
    tokio::spawn(async move {
        let _permit = permit;
        let is_latest = latest_store_operation_versions()
            .lock()
            .expect("latest store operation lock poisoned")
            .get(&freshness_key)
            .copied()
...
#[cfg(feature = "persistence")]
pub async fn run_latest_store_operation<F, Fut, E>(
    store: &Option<EdenStore>,
    persistence_limit: &Arc<Semaphore>,
    config: &RuntimeInfraConfig,
    label: &'static str,
...
pub async fn persist_market_knowledge_projection(
    ...
    schedule_latest_store_operation(
        ...
        format!("macro_event_state:{market_macro_state}"),
        i128::from(tick_number),
        move |store_ref| {
    ...
    schedule_latest_store_operation(
        ...
        format!("knowledge_link_state:{market_link_state}"),
        i128::from(tick_number),
        move |store_ref| {
    ...
    schedule_latest_store_operation(
        ...
        format!("knowledge_event_state:{market_event_state}"),
        i128::from(tick_number),
        move |store_ref| {
    ...
    schedule_latest_store_operation(
        ...
        format!("knowledge_node_state:{market_node_state}"),
        i128::from(tick_number),
        move |store_ref| {
