...
const POLYMARKET_HTTP_TIMEOUT: Duration = Duration::from_secs(10);
...
impl PolymarketMarketConfig {
    pub fn scope(&self) -> ReasoningScope {
        ...
        match self.scope_kind {
            ...
            PolymarketScopeKind::Sector => ReasoningScope::Sector(value.into()),
            PolymarketScopeKind::Theme => ReasoningScope::Theme(value.into()),
            PolymarketScopeKind::Region => ReasoningScope::Region(value.into()),
            PolymarketScopeKind::Custom => ReasoningScope::Custom(value.into()),
        }
    }
}
...
pub async fn fetch_polymarket_snapshot(
    ...
    let client = Client::builder()
        ...
        .timeout(POLYMARKET_HTTP_TIMEOUT)
        .build()
...
async fn fetch_market_prior(
    ...
    let index = yes_outcome_index(&outcomes).ok_or_else(|| {
        format!(
            "market {} does not expose a canonical Yes outcome: {:?}",
            config.slug, outcomes
        )
    })?;
...
mod tests {
    ...
    #[test]
    fn yes_outcome_requires_explicit_yes_label() {
        assert_eq!(
            yes_outcome_index(&["Republican".into(), "Democrat".into()]),
            None
        );
        assert_eq!(yes_outcome_index(&["No".into(), "Yes".into()]), Some(1));
    }
}
