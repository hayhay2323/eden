    ...
    let mut dynamics = series_by_slug
        ...
        .filter_map(|(slug, series)| {
            ...
            let prev_prev = series
                ...
                .and_then(|index| series.get(index))
                .map(|(_, probability)| *probability);
            let probability_delta = prev
                .map(|probability| current_probability - probability)
                .unwrap_or(Decimal::ZERO);
            let prev_delta = match (prev, prev_prev) {
...
mod tests {
    ...
    #[test]
    fn first_polymarket_observation_has_zero_delta() {
        let mut h = TickHistory::new(10);
        h.push(TickRecord {
            tick_number: 1,
            timestamp: OffsetDateTime::UNIX_EPOCH,
            ...
            derived_signals: vec![],
            action_workflows: vec![],
            polymarket_priors: vec![crate::external::polymarket::PolymarketPrior {
                slug: "fed-cut".into(),
                label: "Fed cut".into(),
                question: "Will the Fed cut?".into(),
                scope: crate::ontology::ReasoningScope::market(),
                target_scopes: vec![],
                bias: crate::external::polymarket::PolymarketBias::Neutral,
                selected_outcome: "Yes".into(),
                probability: dec!(0.62),
                conviction_threshold: dec!(0.6),
                active: true,
                closed: false,
                category: None,
                volume: None,
        ...
        let dynamics = compute_polymarket_dynamics(&h);
        assert_eq!(dynamics.len(), 1);
        assert_eq!(dynamics[0].probability_delta, Decimal::ZERO);
        assert_eq!(dynamics[0].probability_acceleration, Decimal::ZERO);
    }
}
