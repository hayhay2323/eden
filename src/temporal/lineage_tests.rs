...
use crate::temporal::analysis::SignalDynamics;
...
fn family_context_outcomes_group_by_family_session_and_regime() {
    ...
    assert_eq!(outcomes[0].session, "after_hours");
...
fn aggregate_outcomes_by_family_takes_horizon_bucket() {
    ...
    // Type-check test: empty input → empty output; the signature must accept HorizonBucket
    for bucket in [
        ...
        let items = aggregate_outcomes_by_family(bucket, vec![]);
        assert!(items.is_empty());
    }
}

#[test]
fn hk_momentum_allow_auto_enter_blocks_collapsing_symbol() {
    let mut t = HkSignalMomentumTracker::default();
    let sym = Symbol("700.HK".into());
    for inst in [dec!(10), dec!(7), dec!(2)] {
        t.record_tick(&SignalDynamics {
            symbol: sym.clone(),
            composite_delta: Decimal::ZERO,
            convergence_delta: Decimal::ZERO,
            composite_acceleration: Decimal::ZERO,
            composite_duration: 0,
            inst_alignment_delta: inst,
            bid_wall_delta: Decimal::ZERO,
            ask_wall_delta: Decimal::ZERO,
            buy_ratio_trend: Decimal::ZERO,
        });
    }
    assert_eq!(t.health(&sym), MomentumHealth::Collapsing);
    let mut actionable = HashSet::new();
    actionable.insert(sym.clone());
    let allowed = t.allow_auto_enter(&actionable);
    assert!(!allowed.contains(&sym));

    let t_fresh = HkSignalMomentumTracker::default();
    let mut a2 = HashSet::new();
    a2.insert(sym.clone());
    assert!(t_fresh.allow_auto_enter(&a2).contains(&sym));
}
