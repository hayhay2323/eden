...
pub fn compute_us_residual_field(
    ...
    for (symbol, convergence) in convergence_scores {
        ...
        let flow_residual = convergence.capital_flow_direction
            - convergence.dimension_composite * Decimal::new(5, 1);
        ...
        if magnitude > Decimal::new(5, 2) {
            residuals.push(SymbolResidual {
                ...
                price_residual,
                flow_residual,
                institutional_residual: cross_market_residual,
                magnitude,
                dominant_dimension,
                net_direction,
            });
...
pub fn compute_us_residual_field_from_canonical(
    convergence_scores: &HashMap<Symbol, UsConvergenceScore>,
    snapshot: &CanonicalMarketSnapshot,
) -> ResidualField {
    let stock_deltas: HashMap<Symbol, Decimal> = snapshot
        .quotes
        .iter()
        .filter(|(_, quote)| quote.prev_close > Decimal::ZERO)
        .map(|(symbol, quote)| {
            let delta = (quote.last_done - quote.prev_close) / quote.prev_close;
            (symbol.clone(), delta)
        })
        .collect();
    ...
    for (symbol, convergence) in convergence_scores {
        ...
        let cross_market_residual = convergence
            .cross_market_propagation
            .map(|prop| observed_delta.signum() * Decimal::new(5, 1) - prop)
            .unwrap_or(Decimal::ZERO);
...
fn compute_sector_expected(stock_deltas: &HashMap<Symbol, Decimal>) -> HashMap<SectorId, Decimal> {
...
fn detect_us_sector_clusters(residuals: &[SymbolResidual]) -> Vec<SectorResidualCluster> {
    ...
    for (sector, items) in &by_sector {
        ...
        let mean_net: Decimal = items.iter().map(|r| r.net_direction).sum::<Decimal>()
            / Decimal::from(items.len() as i64);
        let same_sign = items
            .iter()
...
mod tests {
    ...
    #[test]
    fn us_residual_fourth_component_is_cross_market_not_institutional() {
        let components = [dec!(0.1), dec!(0.2), dec!(0.3), dec!(0.9)];

        assert_eq!(dominant_dim(&components), ResidualDimension::CrossMarket);
        assert_eq!(dominant_dim(&components).label(), "cross_market");
    }
}
