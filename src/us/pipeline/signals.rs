...
fn us_candle_range_normalizer() -> Decimal {
    Decimal::new(8, 2)
}
...
impl UsObservationSnapshot {
    pub fn from_canonical_market(snapshot: &CanonicalMarketSnapshot) -> Self {
        ...
        for quote in snapshot.quotes.values() {
            // Skip quotes without a valid prev_close — these are typically
            // pre-open shells with no reference price. Including them poisons
            // downstream change-rate and gap computations.
        ...
        for (symbol, lines) in &snapshot.capital_flow_series {
            ...
            observations.push(Observation::new(
                ...
                provenance(
                    ...
                    snapshot.timestamp,
                    Some(
                        ((last.inflow * Decimal::from(10_000)).abs() / Decimal::new(1_000_000, 0))
                            .min(Decimal::ONE),
                    ),
        ...
        for (symbol, idx) in &snapshot.calc_indexes {
            observations.push(Observation::new(
        ...
        for (symbol, candles) in &snapshot.candlesticks {
            ...
            let range_ratio = if first.open > Decimal::ZERO {
                crate::math::clamp_signed_unit_interval(
                    (window_high - window_low) / first.open / us_candle_range_normalizer(),
                )
            } else {
                Decimal::ZERO
            };
        ...
        // Preserve a minimal trade-derived quote anchor for symbols with no direct quote.
        for (symbol, trades) in &snapshot.trades {
            if snapshot.quotes.contains_key(symbol) || trades.is_empty() {
                continue;
            }
            let last_done = trades
                .last()
                .map(|trade| trade.price)
                .unwrap_or(Decimal::ZERO);
            let turnover = trades.iter().fold(Decimal::ZERO, |acc, trade| {
            ...
            observations.push(Observation::new(
                ...
                provenance(
                    ProvenanceSource::WebSocket,
                    snapshot.timestamp,
                    Some(confidence_from_turnover(turnover)),
                    [format!("trade_fallback:{}", symbol)],
                ),
            ));
        }
...
impl UsEventSnapshot {
    pub fn detect_from_canonical(
        ...
        let quotes = snapshot
            .quotes
            .iter()
            .map(|(symbol, quote)| crate::ontology::links::QuoteObservation {
                symbol: symbol.clone(),
                last_done: quote.last_done,
                prev_close: quote.prev_close,
                open: quote.open,
        ...
        let calc_indexes = snapshot
            ...
            .iter()
            .map(
                |(symbol, idx)| crate::ontology::links::CalcIndexObservation {
                    symbol: symbol.clone(),
        ...
        let capital_flows = snapshot
            ...
            .filter_map(|(symbol, lines)| {
                lines
                    .last()
                    .map(|line| crate::ontology::links::CapitalFlow {
                        symbol: symbol.clone(),
                        net_inflow: crate::ontology::links::YuanAmount::from_ten_thousands(
                            line.inflow,
                        ),
                    })
            })
...
#[allow(dead_code)]
pub(crate) fn us_propagation_absence_sectors(events: &UsEventSnapshot) -> Vec<SectorId> {
    events
        .events
        .iter()
