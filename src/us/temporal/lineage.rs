...
use crate::ontology::reasoning::{direction_from_setup, ReasoningScope, TacticalDirection};
...
pub fn compute_us_lineage_stats(history: &UsTickHistory, resolution_lag: u64) -> UsLineageStats {
    ...
    for record in &records {
        for setup in &record.tactical_setups {
            ...
            // V2 Pass 2: family_key removed; lineage stat bucket keyed
            // on family_label (still operator-readable; semantically
            // matches the deprecated family_key for downstream lineage
            // rollups).
            let template = record
                .hypotheses
                .iter()
            ...
            let direction: i8 = setup_direction_sign(setup);
...
pub fn compute_us_resolved_topology_outcomes(
    ...
    let mut outcomes = records
        ...
        .filter_map(|(entry_record, setup)| {
            ...
            let net_return = if setup_direction_sign(setup) < 0 {
                -raw_return
            } else {
                raw_return
            };
...
fn setup_direction_sign(setup: &crate::ontology::reasoning::TacticalSetup) -> i8 {
...
impl SignalMomentumTracker {
    ...
    /// Debug helper: return the raw value history for a symbol's convergence
    /// and volume_spike tracks. Lets tests and ad-hoc debugging verify that
    /// the tracker is refreshing per-tick and not serving stale cached values.
    /// Returns (convergence_history, volume_history), each newest-last.
    pub fn debug_history(&self, symbol: &Symbol) -> (Vec<Decimal>, Vec<Decimal>) {
        let conv = self
            .convergence
            .get(symbol)
    ...
    /// Debug helper: format a one-line summary of a symbol's momentum state
    /// including raw values, velocity, and acceleration. Example output:
    ///   "MARA.US conv=[0.42, 0.40, 0.38] vel=-0.020 acc=0.000"
    /// Useful for `println!` debugging inside a tick loop.
    pub fn debug_summary_line(&self, symbol: &Symbol) -> String {
        match self.convergence.get(symbol) {
            None => format!("{:?} conv=[] (no history)", symbol),
            Some(entry) => {
                ...
                format!(
                    ...
                    symbol,
                    values.join(", "),
                    entry.velocity().round_dp(4),
                    entry.acceleration().round_dp(4),
                    entry.values.len()
                )
            }
        }
    ...
    /// Debug helper: return a sorted dump of all tracked symbols and their
    /// most recent convergence value + history length. Handy for quickly
    /// spotting whether the tracker has gone stale globally.
    pub fn debug_all_symbols(&self) -> Vec<(Symbol, usize, Decimal)> {
        let mut rows: Vec<(Symbol, usize, Decimal)> = self
            .convergence
            .iter()
            .map(|(symbol, entry)| {
                let latest = entry.values.last().copied().unwrap_or(Decimal::ZERO);
                (symbol.clone(), entry.values.len(), latest)
            })
            .collect();
...
mod tests {
    ...
    use crate::ontology::reasoning::{
        default_case_horizon, DecisionLineage, Hypothesis, ReasoningScope, TacticalSetup,
    };
    use crate::us::graph::decision::UsMarketRegimeBias;
    ...
    #[test]
    fn debug_helpers_expose_momentum_history_for_inspection() {
        let mut tracker = SignalMomentumTracker::default();
        tracker.record_convergence(sym("AAPL.US"), dec!(0.30));
        tracker.record_convergence(sym("AAPL.US"), dec!(0.42));
        tracker.record_convergence(sym("AAPL.US"), dec!(0.55));
        tracker.record_volume_spike(sym("AAPL.US"), dec!(2.5));

        let (conv_hist, vol_hist) = tracker.debug_history(&sym("AAPL.US"));
        assert_eq!(conv_hist, vec![dec!(0.30), dec!(0.42), dec!(0.55)]);
        assert_eq!(vol_hist, vec![dec!(2.5)]);

        let (empty_conv, empty_vol) = tracker.debug_history(&sym("UNKNOWN.US"));
        assert!(empty_conv.is_empty());
        assert!(empty_vol.is_empty());

        let summary = tracker.debug_summary_line(&sym("AAPL.US"));
        assert!(
            summary.contains("AAPL.US"),
            "summary missing symbol: {}",
            summary
        );
        // rust_decimal preserves minimum scale, so velocity 0.13 formats as
        // "0.13" rather than "0.1300". Accept either form.
        assert!(
            summary.contains("vel=0.13"),
            "unexpected vel format: {}",
            summary
        );
        assert!(
            summary.contains("len=3"),
            "unexpected len format: {}",
            summary
        );

        let all = tracker.debug_all_symbols();
        assert_eq!(all.len(), 1);
        assert_eq!(all[0].0, sym("AAPL.US"));
        assert_eq!(all[0].1, 3);
        assert_eq!(all[0].2, dec!(0.55));
    }
    ...
    #[test]
    fn map_us_lineage_horizon_takes_horizon_bucket() {
        // Type-check test: empty stats → empty output; signature must accept HorizonBucket
        for bucket in [
            HorizonBucket::Tick50,
            HorizonBucket::Fast5m,
            HorizonBucket::Mid30m,
            ...
            let empty_stats = UsLineageStats {
                by_template: vec![],
                by_context: vec![],
            };
            let items = map_us_lineage_horizon(bucket, empty_stats);
