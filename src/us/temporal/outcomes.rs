...
fn us_session_label(timestamp: OffsetDateTime) -> &'static str {
    // US market: 09:30-16:00 ET → 13:30-20:00 UTC (EST, UTC-5)
    // Convert to ET-equivalent minutes by subtracting 5 hours (300 min) from UTC.
    // Note: DST is intentionally ignored for simplicity (matches TimestampSessionResolver).
    use crate::ontology::horizon::SessionPhase;
    let total_minutes = timestamp.hour() as i32 * 60 + timestamp.minute() as i32;
    // Shift to ET: UTC - 5h
    let et_minutes = (total_minutes - 300).rem_euclid(24 * 60) as u16;
    let phase = match et_minutes {
        // 04:00–09:30 ET = 240–569 min
        240..=569 => SessionPhase::PreMarket,
        // 09:30–10:30 ET = 570–629 min
        570..=629 => SessionPhase::Opening,
        // 10:30–15:00 ET = 630–899 min
        630..=899 => SessionPhase::Midday,
        // 15:00–16:00 ET = 900–959 min
        900..=959 => SessionPhase::Closing,
        // 16:00–20:00 ET = 960–1199 min
        960..=1199 => SessionPhase::AfterHours,
...
fn evaluate_us_setup_outcome(
    ...
    let path_returns: Vec<Decimal> = future_records
        .iter()
        .filter_map(|record| {
            let exit_price = record.signals.get(symbol).and_then(us_effective_price)?;
...
pub fn compute_us_case_realized_outcomes_adaptive(
    ...
    let current_tick = window.last().map(|r| r.tick_number).unwrap_or_default();
        ...
        .filter_map(|context| {
            ...
            let resolved_tick = adaptive_peak_tick(
                ...
                future_records.iter().filter_map(|r| {
                    let price = r.signals.get(symbol).and_then(us_effective_price)?;
...
#[allow(deprecated)]
pub(crate) fn evaluate_us_case_outcome_until(
    history: &UsTickHistory,
    setup_id: &str,
    ...
    let context = seen.remove(setup_id)?;
    let future_records = window
        .iter()
        .copied()
        .filter(|record| record.tick_number > context.entry_tick && record.timestamp <= resolved_at)
        .collect::<Vec<_>>();
    evaluate_us_setup_outcome(&context, &future_records)
}
