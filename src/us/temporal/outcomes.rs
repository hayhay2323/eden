...
/// Compute vortex success fingerprints for US — mirrors HK's
/// `compute_vortex_successful_fingerprints`.
///
/// US doesn't have WorldState vortices directly, so we extract from
/// convergence hypotheses and their outcomes.
pub fn compute_us_vortex_successful_fingerprints(
    history: &UsTickHistory,
    limit: usize,
) -> Vec<crate::temporal::lineage::VortexOutcomeFingerprint> {
    use crate::temporal::lineage::VortexOutcomeFingerprint;

    let outcomes = compute_us_case_realized_outcomes_adaptive(history, limit);
    let window = history.latest_n(limit);
    let by_tick: HashMap<u64, &UsTickRecord> = window
        .iter()
        .map(|r| (r.tick_number, *r))
        .collect();

    let mut fingerprints: Vec<VortexOutcomeFingerprint> = outcomes
        .iter()
        .filter(|o| o.net_return > Decimal::ZERO && o.followed_through)
        .filter_map(|outcome| {
            let entry_record = by_tick.get(&outcome.entry_tick)?;
            let setup = entry_record
                .tactical_setups
                .iter()
                .find(|s| s.setup_id == outcome.setup_id)?;
            let hypothesis = entry_record
                .hypotheses
                .iter()
                .find(|h| h.hypothesis_id == setup.hypothesis_id)?;

            // Extract channel info from hypothesis evidence
            let dominant_channels: Vec<String> = hypothesis
                .evidence
                .iter()
                .map(|e| format!("{:?}", e.kind))
            ...
            let channel_signature = {
                let mut sorted = dominant_channels.clone();
                sorted.sort();
                sorted.join("+")
            };

            let center_scope = match &setup.scope {
                ReasoningScope::Symbol(s) => s.0.clone(),
                ReasoningScope::Sector(s) => s.0.clone(),
                _ => "market".into(),
            };
            let center_kind = match &setup.scope {
                ReasoningScope::Symbol(_) => "symbol",
            ...
            Some(VortexOutcomeFingerprint {
                ...
                role: "convergence".into(),
                channel_signature,
                dominant_channels,
                channel_diversity: hypothesis.evidence.len(),
                path_count: hypothesis.propagation_path_ids.len(),
                strength: hypothesis.confidence,
                coherence: hypothesis.local_support_weight,
    ...
    fingerprints.sort_by(|a, b| {
        b.resolved_tick
            .cmp(&a.resolved_tick)
            .then_with(|| b.net_return.cmp(&a.net_return))
    });
    fingerprints
}
