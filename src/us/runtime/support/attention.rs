...
pub(in crate::us::runtime) fn merge_us_standard_attention_maintenance(
    ...
    reasoning_snapshot.hypothesis_tracks = crate::pipeline::reasoning::derive_hypothesis_tracks(
        timestamp,
...
pub(in crate::us::runtime) fn derive_us_vortex_attention(
    ...
    for hypothesis in hypotheses
        .iter()
        .filter(|item| item.family_key == "convergence_hypothesis")
    {
...
mod tests {
    ...
    use crate::ontology::reasoning::{DecisionLineage, HypothesisTrackStatus, PropagationStep};
    ...
    fn merge_us_standard_attention_maintenance_preserves_previous_symbol_state() {
        ...
        assert!(reasoning_snapshot
            .hypothesis_tracks
            .iter()
            .any(|item| item.track_id == previous_track.track_id));
    }
}
