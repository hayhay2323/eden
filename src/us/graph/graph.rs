...
pub const US_STOCK_TO_STOCK_KNN_LIMIT: usize = 20;
...
fn us_theme_groups() -> Vec<Vec<&'static str>> {
    vec![
        // AI / Machine Learning
        vec![
            "NVDA.US", "AMD.US", "AVGO.US", "MRVL.US", "ARM.US", "PLTR.US", "AI.US", "SMCI.US",
            "VRT.US", "SOUN.US", "UPST.US", "CRWD.US", "DDOG.US", "SNOW.US", "PATH.US", "PANW.US",
            "NOW.US", "MSFT.US", "GOOGL.US", "META.US", "AMZN.US", "DELL.US", "HPE.US",
        ],
        // Space / Aerospace
        vec![
            "RKLB.US", "ASTS.US", "LUNR.US", "BA.US", "LMT.US", "NOC.US", "RTX.US", "GD.US",
            "LHX.US", "HII.US", "AXON.US",
        ],
        // Quantum Computing
        vec!["IONQ.US", "RGTI.US", "QUBT.US", "IBM.US", "GOOGL.US"],
        // Fintech / Digital Finance
        vec![
            "COIN.US", "HOOD.US", "SOFI.US", "AFRM.US", "UPST.US", "NU.US", "SQ.US", "PYPL.US",
            "MELI.US",
        ],
        // EV / Clean Energy
        vec![
            "TSLA.US", "RIVN.US", "LCID.US", "NIO.US", "XPEV.US", "LI.US", "ENPH.US", "FSLR.US",
            "CELH.US",
        ],
        // Crypto / Digital Assets
        vec![
            "MSTR.US", "MARA.US", "RIOT.US", "BITF.US", "CLSK.US", "COIN.US", "IBIT.US", "BITO.US",
        ],
        // SE Asia / LatAm emerging tech
        vec!["SE.US", "GRAB.US", "NU.US", "MELI.US"],
        // China ADR tech
        vec![
            "BABA.US", "BIDU.US", "JD.US", "PDD.US", "TCOM.US", "NTES.US", "BILI.US", "TME.US",
            "NIO.US", "XPEV.US", "LI.US",
        ],
        // Social / Consumer Internet
        vec![
            "META.US", "SNAP.US", "PINS.US", "RDDT.US", "RBLX.US", "SPOT.US", "DKNG.US", "DUOL.US",
            "HIMS.US",
        ],
        // Cybersecurity
        vec![
        ...
        // Cloud / SaaS
        vec![
            "CRM.US", "NOW.US", "WDAY.US", "HUBS.US", "DDOG.US", "MDB.US", "SNOW.US", "BILL.US",
            "MNDY.US", "VEEV.US", "TEAM.US",
        ],
        // GLP-1 / Obesity / Biotech
        vec![
            "LLY.US", "NVO.US", "AMGN.US", "VRTX.US", "REGN.US", "MRNA.US", "ALNY.US", "HIMS.US",
        ],
    ]
}
...
impl UsGraph {
    ...
    pub fn compute(
        ...
        for pairs in grouped_pairs.values() {
            ...
            let filtered_pairs: Vec<(usize, usize, Decimal)> = pairs
                .iter()
                .copied()
                .filter(|(_, _, similarity)| *similarity >= cutoff)
                .collect();
            for (i, j, similarity) in top_k_directed_similarity_pairs(
                &filtered_pairs,
                stock_syms.len(),
                US_STOCK_TO_STOCK_KNN_LIMIT,
            ) {
                ...
                graph.add_edge(idx_a, idx_b, UsEdgeKind::StockToStock(edge));
            }
        }
...
fn top_k_directed_similarity_pairs(
    all_pairs: &[(usize, usize, Decimal)],
    stock_count: usize,
    limit: usize,
) -> Vec<(usize, usize, Decimal)> {
    if limit == 0 {
        return Vec::new();
    }

    let mut by_source: Vec<Vec<(usize, Decimal)>> = vec![Vec::new(); stock_count];
    for (i, j, similarity) in all_pairs {
        by_source[*i].push((*j, *similarity));
        by_source[*j].push((*i, *similarity));
    }

    let mut selected = Vec::new();
    for (source, candidates) in by_source.iter_mut().enumerate() {
        candidates.sort_by(|(a_idx, a_sim), (b_idx, b_sim)| {
            b_sim.abs().cmp(&a_sim.abs()).then_with(|| a_idx.cmp(b_idx))
        });
        selected.extend(
            candidates
                .iter()
                .take(limit)
                .map(|(target, similarity)| (source, *target, *similarity)),
        );
    }
    selected
}
...
mod tests {
    ...
    #[test]
    fn graph_caps_stock_similarity_edges_to_top_k_per_source() {
        let entries: Vec<(Symbol, UsSymbolDimensions)> = (0..(US_STOCK_TO_STOCK_KNN_LIMIT + 5))
            .map(|i| {
                (
                    sym(&format!("TST{i:02}.US")),
                    make_dims(dec!(0.6), dec!(0.6), dec!(0.6), dec!(0.0), dec!(0.0)),
                )
            })
            .collect();
        let sector_map: HashMap<Symbol, SectorId> = entries
            .iter()
            .map(|(symbol, _)| (symbol.clone(), SectorId("test_group".into())))
            .collect();
        let snap = make_snapshot(entries);
        let graph = UsGraph::compute(&snap, &sector_map, &HashMap::new());

        for stock_idx in graph.stock_nodes.values() {
            let outgoing = graph
                .graph
                .edges_directed(*stock_idx, petgraph::Direction::Outgoing)
                .filter(|edge| matches!(edge.weight(), UsEdgeKind::StockToStock(_)))
                .count();
            assert!(
                outgoing <= US_STOCK_TO_STOCK_KNN_LIMIT,
                "US stock similarity outdegree must be capped at top-K, got {outgoing}",
            );
        }
    }
