...
pub(super) fn build_hk_action_stage(
    ...
    hk_momentum: &eden::temporal::lineage::HkSignalMomentumTracker,
) -> HkActionStage {
    let actionable_setups = reasoning_snapshot
        .tactical_setups
        .iter()
        .filter(|setup| matches!(setup.action, crate::ontology::TacticalAction::Enter))
        .collect::<Vec<_>>();
    ...
    // Skip auto-enter on Peaking/Collapsing (Palantir 2nd-derivative) — US parity
    // (`us/runtime.rs` + `SignalHealth`); uses momentum state through end of
    // previous tick (see `HkSignalMomentumTracker::allow_auto_enter` doc).
    let momentum_gated: HashSet<Symbol> = hk_momentum.allow_auto_enter(&actionable_symbols);

    let newly_entered =
        tracker.auto_enter_allowed(ready_convergence_scores, Some(&momentum_gated), brain);
...
mod tests {
    use super::*;
    use eden::ontology::reasoning::default_case_horizon;
