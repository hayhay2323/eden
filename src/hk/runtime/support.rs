    ...
    for setup in previous_setups {
        ...
        if carries_symbol_scope && seen_setups.insert(setup.setup_id.clone()) {
            reasoning_snapshot.tactical_setups.push(
                eden::ontology::reasoning::sanitize_carried_tactical_setup(setup),
            );
        }
    }
...
pub(super) fn setup_action_priority(action: &eden::ontology::reasoning::TacticalAction) -> i32 {
    match action {
        eden::ontology::reasoning::TacticalAction::Enter => 0,
        eden::ontology::reasoning::TacticalAction::Review => 1,
        eden::ontology::reasoning::TacticalAction::Observe => 2,
    }
}
