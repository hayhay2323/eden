...
async fn load_runtime_artifacts(market: MarketId) -> Vec<ArtifactHealth> {
    ...
    for (label, path) in runtime_run_artifact_specs(market) {
        items.push(build_artifact_health(label, &path).await);
    }
    items
}

fn runtime_run_artifact_specs(market: MarketId) -> Vec<(&'static str, String)> {
    let store = RuntimeArtifactStore::default();
    [
        RuntimeArtifactKind::RuntimeStageTrace,
        RuntimeArtifactKind::VisualGraphFrame,
        RuntimeArtifactKind::BpMarginals,
        RuntimeArtifactKind::BpMessageTrace,
        RuntimeArtifactKind::SubKgSnapshot,
        RuntimeArtifactKind::RuntimeHealthTick,
    ]
    .into_iter()
    .map(|kind| (kind.label(), store.path(kind, market).display().to_string()))
    .collect()
}
...
mod tests {
    ...
    #[test]
    fn runtime_health_tracks_key_run_artifacts() {
        let specs = runtime_run_artifact_specs(MarketId::Us);
        let labels: Vec<&str> = specs.iter().map(|(label, _)| *label).collect();

        assert!(labels.contains(&"runtime_stage_trace"));
        assert!(labels.contains(&"visual_graph_frame"));
        assert!(labels.contains(&"bp_marginals"));
        assert!(labels.contains(&"runtime_health_tick"));
        assert!(specs.iter().all(|(_, path)| path.starts_with(".run/")));
    }
}
