...
use super::super::ontology_api::{
    get_attention_allocation_detail, get_attention_allocations, get_backward_investigation_sidecar,
    get_case_contract, get_case_contracts, get_macro_event_contract, get_macro_event_contracts,
    get_market_session_contract, get_operational_navigation, get_operational_neighborhood,
    get_operational_snapshot, get_operator_work_item_sidecars, get_organ_overview,
    get_perceptual_evidence, get_perceptual_evidence_detail, get_perceptual_expectation_detail,
    get_perceptual_expectations, get_perceptual_state, get_perceptual_states,
    get_perceptual_uncertainties, get_perceptual_uncertainty_detail, get_recommendation_contract,
    get_recommendation_contracts, get_sector_flow_sidecars, get_symbol_perceptual_attention,
    get_symbol_perceptual_attention_detail, get_symbol_perceptual_evidence,
    get_symbol_perceptual_evidence_detail, get_symbol_perceptual_expectation_detail,
    get_symbol_perceptual_expectations, get_symbol_perceptual_state,
    get_symbol_perceptual_uncertainty, get_symbol_perceptual_uncertainty_detail,
...
use super::super::ontology_history_api::{
    ...
    get_discovered_archetypes, get_recommendation_journal_history, get_workflow_event_history,
};
...
use super::health::{get_live_snapshot, get_us_live_snapshot, health, health_report};
...
pub(in crate::api) fn build_router(state: ApiState) -> Result<Router, ApiError> {
    ...
    let api_routes = Router::new()
        ...
        .route("/ontology/:market/organ", get(get_organ_overview))
        .route(
        ...
        .route(
            "/ontology/:market/perceptual-states",
            get(get_perceptual_states),
        )
        .route(
            "/ontology/:market/perceptual-states/:state_id",
            get(get_perceptual_state),
        )
        // Market-wide perceptual rollups — activated 2026-04-17. The
        // handlers had been written but never wired to the router, so
        // the frontend could only fetch per-symbol slices. These four
        // enable whole-market queries for evidence / expectations /
        // attention / uncertainty, mirroring the per-symbol routes
        // below.
        .route(
            "/ontology/:market/perceptual-evidence",
            get(get_perceptual_evidence),
        )
        .route(
            "/ontology/:market/perceptual-evidence/:evidence_id",
            get(get_perceptual_evidence_detail),
        )
        .route(
            "/ontology/:market/perceptual-expectations",
            get(get_perceptual_expectations),
        )
        .route(
            "/ontology/:market/perceptual-expectations/:expectation_id",
            get(get_perceptual_expectation_detail),
        )
        .route(
            "/ontology/:market/attention-allocations",
            get(get_attention_allocations),
        )
        .route(
            "/ontology/:market/attention-allocations/:allocation_id",
            get(get_attention_allocation_detail),
        )
        .route(
            "/ontology/:market/perceptual-uncertainties",
            get(get_perceptual_uncertainties),
        )
        .route(
            "/ontology/:market/perceptual-uncertainties/:uncertainty_id",
            get(get_perceptual_uncertainty_detail),
        )
        ...
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-state",
            get(get_symbol_perceptual_state),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-evidence",
            get(get_symbol_perceptual_evidence),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-evidence/:evidence_id",
            get(get_symbol_perceptual_evidence_detail),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-expectations",
            get(get_symbol_perceptual_expectations),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-expectations/:expectation_id",
            get(get_symbol_perceptual_expectation_detail),
        )
        .route(
            "/ontology/:market/symbols/:symbol/attention-allocations",
            get(get_symbol_perceptual_attention),
        )
        .route(
            "/ontology/:market/symbols/:symbol/attention-allocations/:allocation_id",
            get(get_symbol_perceptual_attention_detail),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-uncertainties",
            get(get_symbol_perceptual_uncertainty),
        )
        .route(
            "/ontology/:market/symbols/:symbol/perceptual-uncertainties/:uncertainty_id",
            get(get_symbol_perceptual_uncertainty_detail),
        )
        ...
        .route(
            "/ontology/:market/runtime-graph/frame",
            get(get_runtime_graph_frame),
        )
        .route(
            "/ontology/:market/runtime-graph/ego/:symbol",
            get(get_runtime_graph_ego),
        )
        .route(
            "/ontology/:market/runtime-graph/influence/:symbol",
            get(get_runtime_graph_influence),
        )
        .route(
        ...
        .route(
            "/ontology/:market/discovered-archetypes",
            get(get_discovered_archetypes),
        )
        .route(
