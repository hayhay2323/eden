...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match ctx
        ...
        .await
    {
        Ok(list) => println!("  HK overnight securities: {} found", list.len()),
        Err(e) => println!("  HK security_list ERROR: {e}"),
    }
    // Also try None category
    match ctx
        .security_list(
            longport::Market::US,
            None::<longport::quote::SecurityListCategory>,
        )
        .await
    {
        Ok(list) => println!("  US all securities: {} found", list.len()),
