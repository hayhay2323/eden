...
pub struct HypothesisTrackRecord {
    ...
    pub confidence_change: String,
    pub confidence_gap: String,
    pub previous_confidence_gap: Option<String>,
    pub confidence_gap_change: String,
...
impl HypothesisTrackRecord {
    pub fn from_track(track: &HypothesisTrack) -> Self {
        Self {
            ...
            confidence: track.confidence.to_string(),
            previous_confidence: track.previous_confidence.map(|value| value.to_string()),
            confidence_change: track.confidence_change.to_string(),
            confidence_gap: track.confidence_gap.to_string(),
            previous_confidence_gap: track.previous_confidence_gap.map(|value| value.to_string()),
            confidence_gap_change: track.confidence_gap_change.to_string(),
            heuristic_edge: track.heuristic_edge.to_string(),
...
mod tests {
    ...
    fn hypothesis_track_record_preserves_status_and_deltas() {
        ...
        assert_eq!(record.confidence_change, "0.06");
        assert_eq!(record.previous_confidence_gap.as_deref(), Some("0.12"));
        assert_eq!(
