...
pub struct CaseRealizedOutcomeRecord {
    ...
    pub max_favorable_excursion: String,
    pub max_adverse_excursion: String,
    pub followed_through: bool,
    pub invalidated: bool,
    pub structure_retained: bool,
    pub convergence_score: String,
}

impl CaseRealizedOutcomeRecord {
    pub fn from_outcome(outcome: &CaseRealizedOutcome, market: &str) -> Self {
        Self {
            ...
            return_pct: outcome.return_pct.to_string(),
            net_return: outcome.net_return.to_string(),
            max_favorable_excursion: outcome.max_favorable_excursion.to_string(),
            max_adverse_excursion: outcome.max_adverse_excursion.to_string(),
            followed_through: outcome.followed_through,
            invalidated: outcome.invalidated,
            structure_retained: outcome.structure_retained,
            convergence_score: outcome.convergence_score.to_string(),
        }
    }
...
mod tests {
    ...
    fn realized_outcome_record_preserves_key_fields() {
        ...
        let record = CaseRealizedOutcomeRecord::from_outcome(&outcome, "hk");
        assert_eq!(record.market, "hk");
        assert_eq!(record.setup_id, "setup:1");
        assert_eq!(record.net_return, "0.035");
