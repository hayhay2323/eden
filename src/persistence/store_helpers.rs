    ...
    F: FnMut(&T) -> &str,
{
    ...
    let response = db.query(query).bind(("market", market.to_string())).await?;
    // `.check()` collapses a batched-transaction failure into "The query was
    // not executed due to a failed transaction", losing the causing error.
    // When that happens, re-run the delete + each upsert individually so the
    // actual SurrealDB error (type mismatch, constraint violation, etc.) lands
    // in the log exactly once per batch, and surface it instead of the generic
    // message. The slow path only fires when the fast path already failed, so
    ...
    match response.check() {
        ...
        Err(original) => {
            ...
            if let Err(delete_err) = db
                .query(format!("DELETE {table} WHERE market = $market"))
                .bind(("market", market.to_string()))
                .await
                .and_then(|r| r.check())
            {
                let msg = format!("[sync_debug] delete {table} failed: {delete_err}");
                eprintln!("{msg}");
                first_cause.get_or_insert(msg);
            }
            for record in records {
                let id = id_for(record);
                let stmt = upsert_json_query(table, id, record)?;
                if let Err(stmt_err) = db.query(stmt.clone()).await.and_then(|r| r.check()) {
                    let msg = format!("[sync_debug] upsert {table} id={id} failed: {stmt_err}");
                    eprintln!("{msg}");
                    first_cause.get_or_insert(msg);
                    break;
                }
