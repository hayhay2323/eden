...
pub const STOCK_TO_STOCK_KNN_LIMIT: usize = 20;
...
impl BrainGraph {
    ...
    pub fn compute(
        ...
        // 5. Add stock→stock edges (cosine similarity, sparse top-K neighborhood)
        ...
        let filtered_pairs: Vec<(usize, usize, Decimal)> = all_pairs
            .iter()
            .copied()
            .filter(|(_, _, similarity)| similarity.abs() > cutoff)
            .collect();

        // Second pass: each source keeps only its strongest local
        // neighbors. A dense all-pairs similarity matrix is an
        // intermediate statistic, not an ontology graph.
        for (i, j, similarity) in top_k_directed_similarity_pairs(
            &filtered_pairs,
            stock_syms.len(),
            STOCK_TO_STOCK_KNN_LIMIT,
        ) {
...
mod tests {
    ...
    #[test]
    fn stock_similarity_edges_are_sparse_top_k_per_source() {
        let mut narratives = HashMap::new();
        let mut dimensions = HashMap::new();
        for i in 0..(STOCK_TO_STOCK_KNN_LIMIT + 5) {
            let symbol = sym(&format!("{}{:02}.HK", 9000, i));
            narratives.insert(symbol.clone(), make_narrative(dec!(0.5), dec!(0.3)));
            dimensions.insert(
                symbol,
                make_dims(dec!(0.5), dec!(0.5), dec!(0.5), dec!(0.5)),
            );
        ...
        for stock_idx in brain.stock_nodes.values() {
            let outgoing = brain
                .graph
                .edges_directed(*stock_idx, petgraph::Direction::Outgoing)
                .filter(|edge| matches!(edge.weight(), EdgeKind::StockToStock(_)))
                .count();
            assert!(
                outgoing <= STOCK_TO_STOCK_KNN_LIMIT,
                "stock similarity outdegree must be capped at top-K, got {outgoing}",
            );
        }
    }
