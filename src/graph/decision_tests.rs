    ...
    fn all_bullish_convergence() {
        ...
        let score = ConvergenceScore::compute(&sym("700.HK"), &brain, None).unwrap();
    ...
    fn fingerprint_no_degradation() {
        ...
        let fp =
            StructuralFingerprint::capture(&sym("700.HK"), &brain, 7, Some(dec!(320))).unwrap();
    ...
    fn full_degradation() {
        ...
        let mut fp =
            StructuralFingerprint::capture(&sym("700.HK"), &entry_brain, 9, Some(dec!(320)))
                .unwrap();
    ...
    fn degradation_skips_missing_sector_component_in_average() {
        ...
        let fp = StructuralFingerprint::capture(&sym("700.HK"), &entry_brain, 10, Some(dec!(320)))
            .unwrap();
