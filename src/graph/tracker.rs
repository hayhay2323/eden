...
impl PositionTracker {
    ...
    pub fn refresh(&mut self, symbol: &Symbol, brain: &BrainGraph) -> bool {
        if let Some(existing) = self.active.get(symbol) {
            let entry_tick = existing.entry_tick;
            ...
            if let Some(mut fresh) =
                StructuralFingerprint::capture(symbol, brain, entry_tick, entry_price)
            {
                fresh.entry_tick = entry_tick;
                fresh.entry_composite = entry_composite;
                fresh.entry_timestamp = entry_timestamp;
                fresh.entry_price = entry_price;
    ...
    pub fn auto_enter(
        ...
        self.auto_enter_allowed_at(convergence_scores, None, brain, 0, &current_prices)
    }
    ...
    /// Auto-capture fingerprints with live entry metadata threaded in.
    pub fn auto_enter_allowed_at(
        &mut self,
        convergence_scores: &HashMap<Symbol, super::decision::ConvergenceScore>,
        allowed_symbols: Option<&HashSet<Symbol>>,
        brain: &BrainGraph,
        ...
        for (symbol, score) in convergence_scores {
            ...
            if let Some(mut fp) = StructuralFingerprint::capture(
                symbol,
                brain,
                current_tick,
                current_prices.get(symbol).copied(),
            ) {
