...
impl EdgeLearningLedger {
    pub fn from_entries(entries: impl IntoIterator<Item = (EdgeKey, EdgeCredit)>) -> Self {
        Self {
            entries: entries.into_iter().collect(),
        }
    }

    pub fn iter(&self) -> impl Iterator<Item = (&EdgeKey, &EdgeCredit)> {
        self.entries.iter()
    }
...
pub fn ingest_hk_topology_outcomes(
    ...
    let mut credited = 0usize;
    for outcome in
        crate::temporal::lineage::compute_resolved_topology_outcomes(history, resolution_lag)
    {
        if !seen_setup_ids.insert(outcome.setup_id.clone()) {
            continue;
        }
