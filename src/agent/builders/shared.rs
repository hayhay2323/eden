...
pub(super) fn hk_recent_transitions(
    history: &TickHistory,
    store: &ObjectStore,
    window: usize,
) -> Vec<AgentTransition> {
...
pub(super) fn us_recent_transitions(
    ...
    for pair in records.windows(2) {
        ...
        for setup in &current.tactical_setups {
            ...
            transitions.push(AgentTransition {
                ...
                from_state: previous_setup.map(|item| item.action.clone()),
                to_state: setup.action.clone(),
...
pub(super) fn build_hk_structure_state(
    ...
    Some(AgentStructureState {
        ...
        alpha_horizon: Some(alpha_horizon_label(setup.time_horizon.as_str(), 10)),
