...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(transparent)]
pub struct MarketScopeId(pub String);
...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(transparent)]
pub struct CustomScopeId(pub String);
...
impl fmt::Display for MarketScopeId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}
...
impl fmt::Display for CustomScopeId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}
...
impl Default for MarketScopeId {
    fn default() -> Self {
        Self::local()
    }
}

macro_rules! impl_string_newtype_froms {
    ($($ty:ty),+ $(,)?) => {
        $(
...
impl_string_newtype_froms!(SectorId, MarketScopeId, ThemeId, RegionId, CustomScopeId);
