...
impl LinkSnapshot {
    ...
    pub fn from_archive(archive: &TickArchive, store: &ObjectStore) -> Self {
        ...
        let calc_indexes = replay_calc_indexes(archive);
        ...
        let market_temperature = replay_market_temperature(archive);
        let intraday = replay_intraday(archive);
...
fn replay_calc_indexes(archive: &TickArchive) -> Vec<CalcIndexObservation> {
    archive
        .calc_indexes
        .iter()
        .map(|item| CalcIndexObservation {
...
fn replay_market_temperature(archive: &TickArchive) -> Option<MarketTemperatureObservation> {
    archive
        .market_temperature
        .as_ref()
...
fn replay_intraday(archive: &TickArchive) -> Vec<IntradayObservation> {
    let mut by_symbol: HashMap<
        Symbol,
        Vec<&crate::ontology::microstructure::ArchivedIntradayLine>,
    > = HashMap::new();
    for line in &archive.intraday {
        by_symbol.entry(line.symbol.clone()).or_default().push(line);
    }
    ...
    by_symbol
        ...
        .filter_map(|(symbol, lines)| {
            ...
            if latest.avg_price <= Decimal::ZERO {
                return None;
            }
            Some(IntradayObservation {
                ...
                vwap_deviation: (latest.price - latest.avg_price) / latest.avg_price,
