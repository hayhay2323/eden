...
impl KnowledgeNodeKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::PerceptualState => "perceptual_state",
            Self::PerceptualEvidence => "perceptual_evidence",
            Self::PerceptualExpectation => "perceptual_expectation",
            Self::AttentionAllocation => "attention_allocation",
            Self::PerceptualUncertainty => "perceptual_uncertainty",
...
impl From<&str> for KnowledgeNodeKind {
    fn from(value: &str) -> Self {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "perceptual_state" => Self::PerceptualState,
            "perceptual_evidence" => Self::PerceptualEvidence,
            "perceptual_expectation" => Self::PerceptualExpectation,
            "attention_allocation" => Self::AttentionAllocation,
            "perceptual_uncertainty" => Self::PerceptualUncertainty,
...
pub enum KnowledgeRelation {
    ...
    DescribesScope,
    TargetsScope,
    DescribesCurrentStateOf,
    SupportedByEvidence,
    ContradictedByEvidence,
    MissingExpectedEvidenceFor,
    ExpectsConfirmationFrom,
    AttentionAllocatedTo,
    UncertainDueTo,
    CandidateForLeaf,
    LeadingCauseForLeaf,
    TracksSymbol,
...
impl KnowledgeRelation {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::DescribesCurrentStateOf => "describes_current_state_of",
            Self::SupportedByEvidence => "supported_by_evidence",
            Self::ContradictedByEvidence => "contradicted_by_evidence",
            Self::MissingExpectedEvidenceFor => "missing_expected_evidence_for",
            Self::ExpectsConfirmationFrom => "expects_confirmation_from",
            Self::AttentionAllocatedTo => "attention_allocated_to",
            Self::UncertainDueTo => "uncertain_due_to",
...
pub enum KnowledgeNodeAttributes {
    ...
    PerceptualState {
        ...
        age_ticks: u64,
        persistence_ticks: u16,
        direction_continuity_ticks: u16,
        confidence: Decimal,
        strength: Decimal,
        weighted_support_fraction: Decimal,
        count_support_fraction: Decimal,
        support_weight: Decimal,
        contradict_weight: Decimal,
        conflict_age_ticks: u64,
    },
    PerceptualEvidence {
        channel: String,
        polarity: String,
...
mod tests {
    ...
    #[test]
    fn perceptual_link_attributes_serialize_as_tagged_objects() {
        let value = serde_json::to_value(KnowledgeLinkAttributes::DescribesCurrentStateOf {
            state_kind: "continuation".into(),
            trend: "strengthening".into(),
        })
        .unwrap();
        assert_eq!(value["schema"], "describes_current_state_of");
        assert_eq!(value["state_kind"], "continuation");
    }

    #[test]
    fn perceptual_node_attributes_serialize_as_tagged_objects() {
        let value = serde_json::to_value(KnowledgeNodeAttributes::PerceptualState {
            state_kind: "continuation".into(),
            trend: "strengthening".into(),
            direction: Some("buy".into()),
            age_ticks: 5,
            persistence_ticks: 4,
            direction_continuity_ticks: 4,
            confidence: dec!(0.82),
            strength: dec!(0.79),
            weighted_support_fraction: dec!(0.90),
            count_support_fraction: dec!(0.67),
            support_weight: dec!(1.80),
            contradict_weight: dec!(0.20),
            conflict_age_ticks: 0,
        })
        .unwrap();
        assert_eq!(value["schema"], "perceptual_state");
        assert_eq!(value["state_kind"], "continuation");
        assert_eq!(value["trend"], "strengthening");
    }
