...
/// Convert a Rust `Debug` PascalCase label (e.g. `"FailedPropagation"`)
/// into a spaced lowercase label (e.g. `"failed propagation"`).
fn humanize_debug_label(label: &str) -> String {
...
fn summarize_dominant_intents(snapshot: &OperationalSnapshot) -> Vec<String> {
    ...
    for case in &snapshot.cases {
        ...
        let label = humanize_debug_label(&format!("{:?}", intent.kind));
...
fn summarize_dominant_opportunities(snapshot: &OperationalSnapshot) -> Vec<String> {
    let mut grouped = std::collections::HashMap::<
        (String, String),
        (usize, rust_decimal::Decimal, rust_decimal::Decimal),
    >::new();
    ...
    items.into_iter().map(|(_, item)| item).take(5).collect()
}
...
pub fn derive_agent_briefing(snapshot: &OperationalSnapshot) -> AgentBriefing {
    ...
    let dominant_opportunities = summarize_dominant_opportunities(snapshot);
    if let Some(opportunity_summary) = dominant_opportunities.first() {
        let line = format!("dominant opportunity: {opportunity_summary}");
        if !summary.iter().any(|item| item == &line) {
    ...
    AgentBriefing {
        ...
        headline: snapshot
            ...
            .or(pattern_summary),
        summary,
        dominant_intents: summarize_dominant_intents(snapshot),
        spoken_message,
        focus_symbols: snapshot.market_session.focus_symbols.clone(),
        reasons: {
            let mut reasons = snapshot.market_session.wake_reasons.clone();
            if let Some(opportunity_summary) = dominant_opportunities.first() {
                let line = format!("dominant opportunity: {opportunity_summary}");
                if !reasons.iter().any(|item| item == &line) {
