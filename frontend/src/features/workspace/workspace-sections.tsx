...
function CaseRow({ c }: { c: CaseContract }) {
  ...
  const reasoningDetail =
    c.driver_class || c.tension_driver
      ? `${c.driver_class ?? c.tension_driver}${c.lifecycle_phase ? ` · ${c.lifecycle_phase}` : ""}${
          c.peer_confirmation_ratio != null ? ` · peer ${Math.round(c.peer_confirmation_ratio * 100)}%` : ""
        }`
      : null;
  const policyDetail =
    c.multi_horizon_gate_reason || c.policy_reason || c.governance_reason || reasoningDetail || null;
